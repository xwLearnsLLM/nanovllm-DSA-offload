...
## 2026-06-02 18:17:27：精简为 first-block + suffix-block sparse 初始化

下一次请在昇腾上先跑这个：

```bash
# 先在当前 offload 仓目录下执行你平时的模型/TP/HBM/DRAM 通用 export。
# 这次已删除 NANOVLLM_DSA_PREFILL_BUDGET_MODE / NANOVLLM_DSA_DEBUG_PREFILL_BUDGET 等旧 prefill 策略开关。
PYTHONPATH=$PWD:$PYTHONPATH NANOVLLM_MAX_GEN_TOKENS=16 NANOVLLM_ENABLE_DECODE_MLAPO=0 NANOVLLM_DSA_OFFLOAD_FIXED_TX=0 NANOVLLM_DSA_DEBUG_DECODE_METADATA=1 NANOVLLM_DSA_DEBUG_DECODE_METADATA_RANK=0 NANOVLLM_PROFILE_LAYER_IDS=0,mid,last NANOVLLM_PROMPT_LENGTHS=8064,8192,8500,9000,9300 python3 example/test.py
```

## 2026-06-02 18:22:00: first-block plus suffix-block sparse init

Run this next on Ascend:

```bash
# Run your usual model/TP/HBM/DRAM exports first. Old NANOVLLM_DSA_PREFILL_* flags are removed.
PYTHONPATH=$PWD:$PYTHONPATH NANOVLLM_MAX_GEN_TOKENS=16 NANOVLLM_ENABLE_DECODE_MLAPO=0 NANOVLLM_DSA_OFFLOAD_FIXED_TX=0 NANOVLLM_DSA_DEBUG_DECODE_METADATA=1 NANOVLLM_DSA_DEBUG_DECODE_METADATA_RANK=0 NANOVLLM_PROFILE_LAYER_IDS=0,mid,last NANOVLLM_PROMPT_LENGTHS=8064,8192,8500,9000,9300 python3 example/test.py
```
