...
## 2026-06-02 20:03:49：对齐 baseline 热路径，删除两个 MLA 选项和 token0 屏蔽

下一次请在昇腾上先跑这个：

```bash
PYTHONPATH=$PWD:$PYTHONPATH python3 -m py_compile nanovllm/models/deepseek_v32.py nanovllm/layers/activation.py nanovllm/layers/embed_head.py nanovllm/layers/layernorm.py nanovllm/layers/linear.py nanovllm/layers/sampler.py

PYTHONPATH=$PWD:$PYTHONPATH NANOVLLM_MAX_GEN_TOKENS=16 NANOVLLM_ENABLE_DECODE_MLAPO=0 NANOVLLM_DSA_OFFLOAD_FIXED_TX=0 NANOVLLM_PROFILE_LAYER_IDS=mid NANOVLLM_PROMPT_LENGTHS=8064,8192,8500,9000,9300 python3 example/test.py

PYTHONPATH=$PWD:$PYTHONPATH NANOVLLM_MAX_GEN_TOKENS=16 NANOVLLM_ENABLE_DECODE_MLAPO=1 NANOVLLM_DSA_OFFLOAD_FIXED_TX=0 NANOVLLM_PROFILE_LAYER_IDS=mid NANOVLLM_PROMPT_LENGTHS=8064,8192,8500,9000,9300 python3 example/test.py

PYTHONPATH=$PWD:$PYTHONPATH NANOVLLM_MAX_GEN_TOKENS=16 NANOVLLM_ENABLE_DECODE_MLAPO=0 NANOVLLM_DSA_OFFLOAD_FIXED_TX=128 NANOVLLM_DSA_DEBUG_DECODE_METADATA=1 NANOVLLM_DSA_DEBUG_DECODE_METADATA_RANK=0 NANOVLLM_PROFILE_LAYER_IDS=mid NANOVLLM_PROMPT_LENGTHS=8064,8192,8500,9000,9300 python3 example/test.py
```

## 2026-06-02 21:05:02：增加 dsa_index_update 前后 dump 和 scatter 后 KV 校验
