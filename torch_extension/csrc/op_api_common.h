...
// Vendored from the A5 reference adapter and reduced to one queue-safe
// RunOpApiV2 launcher. This repository does not load source files at runtime.
...
{
    const char *explicit_path = std::getenv("NANOVLLM_CUST_OPAPI_LIB");
    if (explicit_path != nullptr && explicit_path[0] != '\0') {
        static auto explicit_handler =
            GetOpApiLibHandler(std::getenv("NANOVLLM_CUST_OPAPI_LIB"));
        TORCH_CHECK(
            explicit_handler != nullptr,
            "cannot load repository-local libcust_opapi.so: ",
            explicit_path);
        auto funcAddr = GetOpApiFuncAddrInLib(
            explicit_handler, explicit_path, apiName);
        if (funcAddr != nullptr) {
            return funcAddr;
        }
    }
