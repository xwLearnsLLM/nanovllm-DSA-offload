...
First, build the standalone `dsa_index_update` CANN op and its Python binding:

```bash
mkdir -p runlog && SOC_VERSION=ascend910_9391 bash scripts/build_dsa_index_update_op.sh 2>&1 | tee runlog/build_dsa_index_update_op_rankfix.txt
```
