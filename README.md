...
First, confirm this source tree contains the AIV-only kernel fix and the new
binding version. The grep should print `KERNEL_TYPE_AIV_ONLY` and
`manual_acl_tensor_aiv_only_v2`:

```bash
grep -n "KERNEL_TYPE_AIV_ONLY\|manual_acl_tensor_aiv_only_v2\|EXEC_NPU_CMD(aclnnDsaIndexUpdate" dsa_index_update_op/cann/op_kernel/dsa_index_update.cpp dsa_index_update_op/torch_extension/dsa_index_update_ext.cpp nanovllm/models/dsa_index_update_real.py
```
