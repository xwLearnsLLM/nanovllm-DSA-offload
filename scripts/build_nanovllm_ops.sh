...
CUSTOM_OPS="fused_li_manage;kvcache_scatter_copy;kvcache_offload_copy;sparse_tail_attention;fused_copy_sfa;moe_gating_top_k;matmul_allreduce_add_rmsnorm"
...
if [[ "${NANOVLLM_SKIP_CANN_OPP_BUILD:-0}" == "1" ]]; then
  ...
  for OP_TYPE in NanovllmFusedLiManage NanovllmKvcacheScatterCopy NanovllmKvcacheOffloadCopy NanovllmSparseTailAttention NanovllmFusedCopySfa; do
    if ! grep -q "${OP_TYPE}" "${BINARY_INFO_CONFIG}"; then
      echo "[nanovllm ops] ERROR: ${OP_TYPE} is missing from ${BINARY_INFO_CONFIG}." >&2
      exit 1
    fi
  done
  echo "[nanovllm ops] verified FUSED_LI_MANAGE/SCATTER/OFFLOAD/SPARSE_TAIL_ATTENTION/FUSED_COPY_SFA kernels in ${BINARY_INFO_CONFIG}"
  popd >/dev/null
fi
