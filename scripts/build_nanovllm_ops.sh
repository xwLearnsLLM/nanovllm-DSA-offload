...
CUSTOM_OPS="lightning_indexer_decode_update;lightning_indexer_decode_update_mtp;kvcache_scatter_copy;sparse_and_tail_attention;sparse_and_tail_attention_mtp;sparse_and_tail_attention_and_scatter_copy;moe_gating_top_k;matmul_allreduce_add_rmsnorm"
...
if [[ "${NANOVLLM_SKIP_CANN_OPP_BUILD:-0}" == "1" ]]; then
  ...
  for OP_TYPE in NanovllmLiduDecodeUpdate NanovllmLiduDecodeUpdateMtp NanovllmKvcacheScatterCopy NanovllmSparseAndTailAttention NanovllmSparseAndTailAttentionMtp NanovllmSparseAndTailAttentionAndScatterCopy; do
    if ! grep -q "${OP_TYPE}" "${BINARY_INFO_CONFIG}"; then
      echo "[nanovllm ops] ERROR: ${OP_TYPE} is missing from ${BINARY_INFO_CONFIG}." >&2
      exit 1
    fi
  done
  echo "[nanovllm ops] verified LIDU/MTP-LIDU/SCATTER/ATTENTION/MTP-ATTENTION/FUSED kernels in ${BINARY_INFO_CONFIG}"
  popd >/dev/null
fi
