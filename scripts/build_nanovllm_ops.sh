...
CUSTOM_OPS="lightning_indexer;gather_selection_kv_cache;lightning_indexer_decode_update;kvcache_scatter_copy;sparse_and_tail_attention;moe_gating_top_k;matmul_allreduce_add_rmsnorm"
...
if [[ "${NANOVLLM_SKIP_CANN_OPP_BUILD:-0}" == "1" ]]; then
  ...
  for OP_TYPE in NanovllmLiduDecodeUpdate NanovllmKvcacheScatterCopy NanovllmSparseAndTailAttention; do
    if ! grep -q "${OP_TYPE}" "${BINARY_INFO_CONFIG}"; then
      echo "[nanovllm ops] ERROR: ${OP_TYPE} is missing from ${BINARY_INFO_CONFIG}." >&2
      exit 1
    fi
  done
  echo "[nanovllm ops] verified LIDU/SCATTER/ATTENTION kernels in ${BINARY_INFO_CONFIG}"
  popd >/dev/null
fi
