#!/usr/bin/env python3
"""Validate bundled GLM SCATTER+SFA against the fused source-aware path."""
...
import nanovllm.ops  # noqa: E402,F401
...
KPE_DIM = 64
SPARSE_COUNT = 2048
DEFAULT_TEST_HEADS = 4
SUPPORTED_TEST_HEADS = (4, 8, 128)
...
def attention_logical_tokens(case: Case) -> torch.Tensor:
    ...
    for row in range(sparse.size(0)):
        ...
        if cache_length == 0:
            logical = torch.arange(actual_length, dtype=torch.int64)
        else:
            tail = torch.arange(
...
def print_attention_diagnostics(
    ...
    print(
        f"FUSED_COPY_SFA_DIAGNOSTIC phase={label} "
...
def check_semantics(case: Case) -> None:
    if case.query.size(0) > 1:
        original_cache_tokens = case.cache_tokens[0].clone()
        original_actual_kv = case.actual_kv[0].clone()
        original_copy_count = case.copy_counts[0].clone()
        tail_tokens = int(original_actual_kv) - int(original_cache_tokens)
        case.cache_tokens[0] = 0
        case.actual_kv[0] = SPARSE_COUNT + tail_tokens
        case.copy_counts[0] = 0
        _, mixed_source_cpu = active_physical_token_indices(
            case.dram_block_table,
            case.source_token_ids,
            case.copy_counts,
        )
        mixed_cpu_golden = build_cpu_attention_golden(
            case,
            mixed_source_cpu,
        )
        mixed_serial = launch_serial(case)
        mixed_fused = launch_fused(case)
        torch.npu.synchronize()
        torch.testing.assert_close(
            mixed_fused.float(),
            mixed_serial.float(),
        ...
        torch.testing.assert_close(
            mixed_fused.cpu().float(),
            mixed_cpu_golden,
            rtol=0.08,
            atol=0.08,
        )
        print(
            "FUSED_COPY_SFA_MIXED_C0_CHECK "
            f"batch={case.query.size(0)} c0_rows=1 ok=1",
            flush=True,
        )
        case.cache_tokens[0].copy_(original_cache_tokens)
        case.actual_kv[0].copy_(original_actual_kv)
        case.copy_counts[0].copy_(original_copy_count)

    original_counts = case.copy_counts.clone()
    case.copy_counts.zero_()

    print("FUSED_COPY_SFA_PHASE zero_miss_serial", flush=True)
    serial_zero_out = launch_serial(case)
    torch.npu.synchronize()
    print("FUSED_COPY_SFA_PHASE zero_miss_fused", flush=True)
    ...
    print(
        "FUSED_COPY_SFA_ZERO_MISS_CHECK "
    ...
    print("FUSED_COPY_SFA_PHASE random_miss_serial", flush=True)
    serial_out = launch_serial(case)
    torch.npu.synchronize()
    print("FUSED_COPY_SFA_PHASE random_miss_fused", flush=True)
    ...
    print(
        "FUSED_COPY_SFA_DRAM_COPY_CHECK "
    ...
    print(
        "FUSED_COPY_SFA_CPU_GOLDEN_CHECK "
...
def benchmark(case: Case, warmup: int, iters: int) -> None:
    ...
    strategy = "source_aware_stagger3"
    ...
    # Measure components after the primary serial/fused comparison so these
    # diagnostics cannot perturb its ordering.
    scatter_ms = event_benchmark(
        lambda: launch_scatter_only(case), warmup, iters
    )
    sfa_ms = event_benchmark(lambda: launch_sfa_only(case), warmup, iters)
    speedup = serial_ms / fused_ms
    print(
        "FUSED_COPY_SFA_RESULT "
...
def graph_replay_check(case: Case, replays: int) -> None:
    if replays <= 0:
        return
    ...
    cpu_golden = build_cpu_attention_golden(case, source_cpu)
    destination = destination_cpu.to(case.query.device)
    expected_ckv = case.dram_ckv_cpu.view(-1, CKV_DIM)[source_cpu]
    expected_kpe = case.dram_kpe_cpu.view(-1, KPE_DIM)[source_cpu]
    ...
    with torch.npu.graph(graph, pool=pool):
        graph_output = launch_fused(case)
    torch.npu.synchronize()

    for replay in range(replays):
        if destination.numel():
            case.fused_ckv.view(-1, CKV_DIM).index_fill_(
                0, destination, 37.0 + replay
            )
            case.fused_kpe.view(-1, KPE_DIM).index_fill_(
                0, destination, -29.0 - replay
            )
        ...
        torch.testing.assert_close(
            case.fused_ckv.view(-1, CKV_DIM)[destination].cpu(),
            expected_ckv,
            rtol=0,
            atol=0,
        )
        torch.testing.assert_close(
            case.fused_kpe.view(-1, KPE_DIM)[destination].cpu(),
            expected_kpe,
            rtol=0,
            atol=0,
        )
        torch.testing.assert_close(
            graph_output.cpu().float(),
            cpu_golden,
            rtol=0.08,
            atol=0.08,
        )
    ...
    print(
        "FUSED_COPY_SFA_GRAPH_CHECK "
        f"batch={case.query.size(0)} replays={replays} "
        f"copied_tokens={destination.numel()} "
        "poison_each_replay=1 source_to_hbm_exact=1 "
        "attention_golden=1 ok=1",
        flush=True,
    )
