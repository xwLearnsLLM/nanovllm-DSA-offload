...
class Result:
    ...
    def management_us(self) -> float:
        return self.lidu_us - self.lightning_indexer_us


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark repository-local LightningIndexer and LIDU without "
            "SCATTER."
        )
    )
    ...
    *,
    ...
    if not torch.equal(counts_cpu, target_misses):
        raise AssertionError(
            f"LIDU miss counts {counts_cpu.tolist()} do not match targets "
...
def run_case(
    *,
    ...
    def run_lidu() -> tuple[torch.Tensor, ...]:
    ...
    lidu_times = benchmark_npu_events(
        run_lidu,
        warmup=warmup,
        iters=iters,
        reset=reset_lidu,
...
def main() -> None:
    ...
    print(
        f"LIDU_PERF_OPAPI path={opapi_path} local=1",
        flush=True,
    )
    print(
        f"LIDU_PERF_CONFIG device={device} heads={args.heads} "
