...
| `fused_copy_sfa_mtp` | 单内核融合 MTP3 source-aware 搬移、HBM 回写与四行 causal sparse Attention | MTP3 |
...
- `fused_copy_sfa` 支持 bs>24；`fused_copy_sfa_mtp` 使用独立 source-aware 单内核实现，并以 `scatter_copy + sparse_tail_attention_mtp` 作为精确语义基线。
...
python3 ut_ops/test_fused_li_manage_mtp.py \
  --device npu:0 --batch-size 24 --source-len 20992 \
  --cache-tokens 8192 --perf-query-miss-count 200 \
  --perf-query-noise 0.25 --graph-replays 3 \
  --warmup 10 --iters 100 --seed 7
