...
#include "a5_qsfa_mtp/kv_quant_sparse_flash_attention_template_tiling_key.h"
#if (__CCE_AICORE__ == 310)
#include "a5_qsfa_mtp/arch35/kv_quant_sparse_flash_attention_kernel_mla.h"
#else
#include "a5_qsfa_mtp/kv_quant_sparse_flash_attention_kernel_mla.h"
#endif
    ...
    __gm__ uint8_t *user, __gm__ uint8_t *tiling, TPipe &tPipe)
{
    if constexpr (ORIG_DTYPE_QUERY == DT_BF16 && ORIG_DTYPE_KEY == DT_FLOAT8_E4M3FN &&
        ...
        QSFA_OP_IMPL(BaseApi::KvQuantSparseFlashAttentionMla, KvQuantSparseFlashAttentionMtpTilingDataMla,
...
a5_sparse_tail_attention_mtp_c8(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                       __gm__ uint8_t *sparseIndices, __gm__ uint8_t* keyScale, __gm__ uint8_t* valueScale,
                       __gm__ uint8_t *blocktable, __gm__ uint8_t *actualSeqLengthsQuery,
                       __gm__ uint8_t *actualSeqLengthsKV, __gm__ uint8_t *attentionOut,
                       ...
                       __gm__ uint8_t *workspace, __gm__ uint8_t *tiling)
{
    ...
    if constexpr (ORIG_DTYPE_QUERY == DT_FLOAT16 && ORIG_DTYPE_KEY == DT_INT8 &&
                  ORIG_DTYPE_ATTENTION_OUT == DT_FLOAT16) {
        QSFA_OP_IMPL(KvQuantSparseFlashAttentionMla, KvQuantSparseFlashAttentionMtpTilingDataMla, half, int8_t,
            half, FLASH_DECODE, static_cast<QSFA_LAYOUT>(LAYOUT_T), static_cast<QSFA_LAYOUT>(KV_LAYOUT_T),
            TEMPLATE_MODE);
    } else { // bf16
        QSFA_OP_IMPL(KvQuantSparseFlashAttentionMla, KvQuantSparseFlashAttentionMtpTilingDataMla, bfloat16_t, int8_t,
