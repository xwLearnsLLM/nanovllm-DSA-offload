...
/*!
 * \file a5_sparse_tail_attention_mtp_c8.cpp
 * \brief MTP variant op definition of the packed-C8 sparse+tail attention.
 *
 * The aclnn ABI, tiling and kernel implementation are identical to
 * A5SparseTailAttentionC8: the MTP semantics (per-verification-query top-2048,
 * causal dense tail, 1..4 query rows per request) are carried entirely by the
 * TND query layout, the packed sparse_and_tail_slots rows and the cumulative
 * actual_seq_lengths_query, all of which the QSFA kernel already handles.
 * The separate op exists so aclnn dispatch, tiling caches and the torch
 * schema stay independently versionable.
 */

#include "register/op_def_registry.h"

namespace ops {
class A5SparseTailAttentionMtpC8 : public OpDef {
public:
    explicit A5SparseTailAttentionMtpC8(const char *name) : OpDef(name)
    {
        this->Input("query")
            .ParamType(REQUIRED)
...
OP_ADD(A5SparseTailAttentionMtpC8);
} // namespace ops
