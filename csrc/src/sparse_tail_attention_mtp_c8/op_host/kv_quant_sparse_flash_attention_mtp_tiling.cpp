...
#include "../op_kernel/a5_qsfa_mtp/kv_quant_sparse_flash_attention_template_tiling_key.h"
#include "kv_quant_sparse_flash_attention_mtp_tiling.h"
...
const std::map<std::string, std::vector<QSFALayoutMtp>> LAYOUT_SUPPORT_MAP = {
    {QUERY_NAME,             {QSFALayoutMtp::BSND, QSFALayoutMtp::TND}},
    {KEY_NAME,               {QSFALayoutMtp::BSND, QSFALayoutMtp::TND, QSFALayoutMtp::PA_BSND}},
    {VALUE_NAME,             {QSFALayoutMtp::BSND, QSFALayoutMtp::TND, QSFALayoutMtp::PA_BSND}},
    {ATTEN_OUT_NAME,         {QSFALayoutMtp::BSND, QSFALayoutMtp::TND}},
};
...
struct KvQuantSparseFlashAttentionMtpCompileInfo {
    int64_t coreNum;
};

static const std::map<QSFALayoutMtp, std::vector<QSFAAxisMtp>> QSFA_LAYOUT_AXIS_MAP = {
    {QSFALayoutMtp::BSND, {QSFAAxisMtp::B, QSFAAxisMtp::S, QSFAAxisMtp::N, QSFAAxisMtp::D}},
    {QSFALayoutMtp::TND, {QSFAAxisMtp::T, QSFAAxisMtp::N, QSFAAxisMtp::D}},
    {QSFALayoutMtp::PA_BSND, {QSFAAxisMtp::Bn, QSFAAxisMtp::Bs, QSFAAxisMtp::N, QSFAAxisMtp::D}},
};

static const std::map<QSFALayoutMtp, size_t> QSFA_LAYOUT_DIM_MAP = {
    {QSFALayoutMtp::BSND, DIM_NUM_FOUR},
    {QSFALayoutMtp::TND, DIM_NUM_THREE},
    {QSFALayoutMtp::PA_BSND, DIM_NUM_FOUR},
};
...
string QSFATensorDesc2StringMtp(const gert::StorageShape *shape, const gert::CompileTimeTensorDesc *tensor)
{
...
string QSFADebugTilingContextMtp(const gert::TilingContext *context)
{
    std::ostringstream qsfaOss;
    for (size_t i = 0; i < context->GetComputeNodeInfo()->GetInputsNum(); ++i) {
        qsfaOss << "input" << i << ": ";
        qsfaOss << QSFATensorDesc2StringMtp(context->GetInputShape(i), context->GetInputDesc(i));
    }

    for (size_t i = 0; i < context->GetComputeNodeInfo()->GetOutputsNum(); ++i) {
        qsfaOss << "output" << i << ": ";
        qsfaOss << QSFATensorDesc2StringMtp(context->GetOutputShape(i), context->GetOutputDesc(i));
    }
    return qsfaOss.str();
}

std::string QSFALayoutMtpToSerialStringMtp(QSFALayoutMtp layout)
{
    switch (layout) {
        case QSFALayoutMtp::BSND: return "BSND";
        case QSFALayoutMtp::TND: return "TND";
        case QSFALayoutMtp::PA_BSND: return "PA_BSND";
...
ge::graphStatus QSFAMlaTilingMtp::SetBlockDim(uint32_t blockDim) const
{
    context_->SetBlockDim(blockDim);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAMlaTilingMtp::SetTilingKey(uint64_t tilingKey) const
{
...
ge::graphStatus QSFAMlaTilingMtp::SetWorkspaceSize(uint64_t workspaceSize) const
{
...
ge::graphStatus QSFAMlaTilingMtp::SetTilingData(TilingDef &tilingData) const
{
...
ge::graphStatus QSFAMlaTilingMtp::GetPlatformInfo()
{
...
void QSFAMlaTilingMtp::GenTilingKey()
{
    ...
    if (qsfaInfo_->kvLayout == QSFALayoutMtp::PA_BSND) {
        pageAttention = 1U;
    }

    tilingKey_ = GET_TPL_TILING_KEY(0U, pageAttention, layoutQuery, layoutKV, \
        perfMode_ == QSFAPerfModeMtp::V_TEMPLATE_MODE, static_cast<uint32_t>(qsfaInfo_->gSize > 64)); // G大于64时核间切G

    OP_LOGI(qsfaInfo_->opName, "QSFA tilingKey_: %lu.", tilingKey_);
}

void QSFAMlaTilingMtp::ZeroTensorProcess() const
{
...
void QSFAMlaTilingMtp::InitParams()
{
    perfMode_ = QSFAPerfModeMtp::V_TEMPLATE_MODE;
    coreNum_ = aicNum_;
...
void QSFAMlaTilingMtp::CalcUbBmm()
{
...
void QSFAMlaTilingMtp::CheckUbSpace()
{
    CalcUbBmm();
}

void QSFAMlaTilingMtp::CalcInnerSize(uint32_t qsfaS2Size)
{
    sInnerSize_ = 512; // 512:s2默认切分大小
    // FlashDecode时，如果S2的计算量>=256(确保切分后不小于128)但又不足以分2次计算时，则修改sInnerSize_，均分为2份进行计算，确保Nbuffer=2
    if (splitKVFlag_ && qsfaInfo_->qLayout != QSFALayoutMtp::TND) {
...
void QSFAMlaTilingMtp::SplitBalanced()
{
    CalcInnerSize(qsfaInfo_->s2Size);
    InnerSplitParamsMtp qsfaInnerSplitParams;
...
void QSFAMlaTilingMtp::Split()
{
    SplitBalanced();
}

void QSFAMlaTilingMtp::FillTilingBaseParamsMla()
{
...
// for flash decode
void QSFAMlaTilingMtp::FillTilingSplitKVMla()
{
...
void QSFAMlaTilingMtp::FillTilingSingleCoreParamsMla()
{
    tilingData_.singleCoreParams.set_usedCoreNum(usedCoreNum_);
}

void QSFAMlaTilingMtp::FillTilingSingleCoreTensorSizeMla()
{
...
void QSFAMlaTilingMtp::FillTiling()
{
...
uint32_t QSFAMlaTilingMtp::CalcBalanceFDParamNums(const uint32_t actCoreNum) const
{
    return actCoreNum * 2 * qsfaInfo_->n2Size * mBaseSize_; // 2:每个核可能有头规约和尾规约，一共两份规约信息
}

void QSFAMlaTilingMtp::NormalCalcFDWorkSpace(const uint32_t actCoreNum)
{
    if (splitKVFlag_) {
...
void QSFAMlaTilingMtp::CalcFDWorkSpace(const uint32_t actCoreNum)
{
    NormalCalcFDWorkSpace(actCoreNum);
}

void QSFAMlaTilingMtp::GetWorkspaceSize()
{
...
void QSFAMlaTilingMtp::CalcBlockDim()
{
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(qsfaInfo_->platformInfo);
...
ge::graphStatus QSFAMlaTilingMtp::DoOpTiling(QSFATilingInfoMtp *qsfaInfo)
{
    qsfaInfo_ = qsfaInfo;
...
ge::graphStatus TilingKvQuantSparseFlashAttentionMtp(gert::TilingContext *context)
{
    QSFATilingInfoMtp qsfaInfo;
    QSFAInfoParserMtp qsfaInfoParser(context);
    if (qsfaInfoParser.Parse(qsfaInfo) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    QSFATilingCheckMtp tilingChecker(qsfaInfo);
    if (tilingChecker.Process() != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    QSFAMlaTilingMtp tiling(context);
    return tiling.DoOpTiling(&qsfaInfo);
}

ge::graphStatus TilingPrepareForKvQuantSparseFlashAttentionMtp(gert::TilingParseContext* const context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFATilingCheckMtp::GetExpectedShape(gert::Shape &shapeExpected,
    const QSFATilingShapeCompareParamMtp &param, const QSFALayoutMtp &layout) const
{
    if (layout == QSFALayoutMtp::BSND) {
        shapeExpected = gert::Shape({param.B, param.S, param.N, param.D});
    } else if (layout == QSFALayoutMtp::TND) {
        shapeExpected = gert::Shape({param.T, param.N, param.D});
    } else if (layout == QSFALayoutMtp::PA_BSND) {
        shapeExpected = gert::Shape({param.Bn, param.Bs, param.N, param.D});
    } else {
        OP_LOGE(opName_, "layout %s is unsupported", QSFALayoutMtpToSerialStringMtp(layout).c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFATilingCheckMtp::CompareShape(QSFATilingShapeCompareParamMtp &param,
    const gert::Shape &shape, const QSFALayoutMtp &layout, const std::string &name) const
{
...
void QSFATilingCheckMtp::LogErrorDtypeSupport(const std::vector<ge::DataType> &expectDtypeList,
...
ge::graphStatus QSFATilingCheckMtp::CheckDtypeSupport(const gert::CompileTimeTensorDesc *qsfaDesc,
...
template <typename T>
void QSFATilingCheckMtp::LogErrorNumberSupport(const std::vector<T> &expectNumberList,
...
template <typename T>
void QSFATilingCheckMtp::LogErrorDimNumSupport(const std::vector<T> &expectNumberList,
    const T &actualValue, const std::string &name) const
{
    LogErrorNumberSupport(expectNumberList, actualValue, name, "dimension");
}

ge::graphStatus QSFATilingCheckMtp::CheckDimNumInLayoutSupport(const QSFALayoutMtp &layout,
    const gert::StorageShape *shape, const std::string &name) const
{
    ...
    OP_CHECK_IF(shape->GetStorageShape().GetDimNum() != qsfaDimIt->second,
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(opName_, name.c_str(),
            ...
            "When layout is " + QSFALayoutMtpToSerialStringMtp(layout) +
            ", the shape dim of " + name + "should be " + std::to_string(qsfaDimIt->second)),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFATilingCheckMtp::CheckDimNumSupport(const gert::StorageShape *shape,
...
void QSFATilingCheckMtp::LogErrorLayoutSupport(const std::vector<QSFALayoutMtp> &expectLayoutList,
    const QSFALayoutMtp &actualLayout, const std::string &name) const
{
    std::ostringstream qsfaOssLayout;
    for (size_t i = 0; i < expectLayoutList.size(); ++i) {
        qsfaOssLayout << QSFALayoutMtpToSerialStringMtp(expectLayoutList[i]);
        if (i < expectLayoutList.size() - 1) {
            qsfaOssLayout << ", ";
        }
    }
    OP_LOGE_FOR_INVALID_FORMAT(opName_, name.c_str(),
        QSFALayoutMtpToSerialStringMtp(actualLayout).c_str(), qsfaOssLayout.str().c_str());
}

ge::graphStatus QSFATilingCheckMtp::CheckLayoutSupport(const QSFALayoutMtp &actualLayout, const std::string &name) const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckSingleParaQuery() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckSingleParaKey() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckSingleParaNumHeads() const
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFATilingCheckMtp::CheckSingleParaKvHeadNums() const
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFATilingCheckMtp::CheckSingleParaSparseMode() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckSingleParaSparseBlockSize() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckSingleParaSparseIndices() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckSinglePara() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckDequantScaleNotExistence()
{
...
template <typename T>
ge::graphStatus QSFATilingCheckMtp::CheckAttrValueByMap(std::map<std::string, std::pair<const T *, T>> &attrMap) const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckParaExistenceMlaAntiquant() const
{
    if (kvLayout_ == QSFALayoutMtp::BSND) {
        return ge::GRAPH_SUCCESS;
    } else if (kvLayout_ == QSFALayoutMtp::TND) {
        OP_CHECK_IF(opParamInfo_.actualSeqLengths.tensor == nullptr,
                   OP_LOGE_WITH_INVALID_INPUT(opName_, "actualSeqLengthsKv"),
                   return ge::GRAPH_FAILED);
    } else if (kvLayout_ == QSFALayoutMtp::PA_BSND) {
        OP_CHECK_IF(opParamInfo_.actualSeqLengths.tensor == nullptr,
                   OP_LOGE_WITH_INVALID_INPUT(opName_, "actualSeqLengthsKv"),
                   return ge::GRAPH_FAILED);
...
ge::graphStatus QSFATilingCheckMtp::CheckParaExistenceMla() const
{
    return CheckParaExistenceMlaAntiquant();
}

ge::graphStatus QSFATilingCheckMtp::CheckParaExistence()
{
...
void QSFATilingCheckMtp::SetQSFAShapeCompare()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckBlockTable() const
{
    if (kvStorageMode_ != KvStorageModeMtp::PAGE_ATTENTION) {
        OP_CHECK_IF(opParamInfo_.blockTable.tensor != nullptr,
            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(opName_, BLOCK_TABLE_NAME.c_str(),
                ...
                "When the layout_kv is " + QSFALayoutMtpToSerialStringMtp(kvLayout_) + ", block_table should be null."),
            return ge::GRAPH_FAILED);
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus QSFATilingCheckMtp::CheckDTypeConsistency(const ge::DataType &actualDtype,
...
ge::graphStatus QSFATilingCheckMtp::CheckTopkShape()
{
    QSFATilingShapeCompareParamMtp qsfaShapeParams;
...
ge::graphStatus QSFATilingCheckMtp::CheckAttenOutShape()
{
    QSFATilingShapeCompareParamMtp shapeParams;
...
ge::graphStatus QSFATilingCheckMtp::CheckAttenOut()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckTopK()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckKVShapeForBatchContinuous()
{
    QSFATilingShapeCompareParamMtp shapeParams;
    shapeParams.B = bSize_;
...
ge::graphStatus QSFATilingCheckMtp::CheckKVShapeForPageAttention()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckKVShape()
{
    if (kvStorageMode_ == KvStorageModeMtp::BATCH_CONTINUOUS) {
...
ge::graphStatus QSFATilingCheckMtp::CheckKV()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckActualSeqLensQ()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckActualSeqLensQDType()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckActualSeqLensQShape()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckActualSeqLens()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckActualSeqLensDType()
{
...
ge::graphStatus QSFATilingCheckMtp::CheckActualSeqLensShape()
{
    if (opParamInfo_.actualSeqLengths.tensor == nullptr) {
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus QSFATilingCheckMtp::CheckMultiParaConsistency()
{
    SetQSFAShapeCompare();
...
ge::graphStatus QSFATilingCheckMtp::CheckFeatureMlaAntiquantShape() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckFeatureMlaAntiquantShapeSizes() const
{
    ...
    OP_CHECK_IF(qTSize_ <= 0 && (qLayout_ == QSFALayoutMtp::TND),
...
ge::graphStatus QSFATilingCheckMtp::CheckFeatureMlaAntiquantShapeSparseAndHeadDim() const
{
    if (isA5_) {
...
ge::graphStatus QSFATilingCheckMtp::CheckFeatureMlaAntiquantLayout() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckFeatureMlaAntiquantDtype() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckFeatureMlaAntiquantAttr() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckFeatureMlaAntiquantPa() const
{
    if (kvStorageMode_ != KvStorageModeMtp::PAGE_ATTENTION) {
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus QSFATilingCheckMtp::CheckFeatureMlaAntiquant() const
{
...
ge::graphStatus QSFATilingCheckMtp::CheckFeatureMla() const
{
    return CheckFeatureMlaAntiquant();
}

ge::graphStatus QSFATilingCheckMtp::CheckFeature() const
{
    return CheckFeatureMla();
}

void QSFATilingCheckMtp::Init()
{
...
ge::graphStatus QSFATilingCheckMtp::Process()
{
    Init();
...
static bool HasAxis(const QSFAAxisMtp &axis, const QSFALayoutMtp &layout, const gert::Shape &shape)
{
    ...
    const std::vector<QSFAAxisMtp>& qsfaAxes = qsfaLayoutIt->second;
...
static size_t GetAxisIdx(const QSFAAxisMtp &axis, const QSFALayoutMtp &layout)
{
    const std::vector<QSFAAxisMtp>& axes = QSFA_LAYOUT_AXIS_MAP.find(layout)->second;
    const auto& axisIt = std::find(axes.begin(), axes.end(), axis);

    return std::distance(axes.begin(), axisIt);
}

static uint32_t GetAxisNum(const gert::Shape &shape, const QSFAAxisMtp &axis, const QSFALayoutMtp &layout)
{
    return HasAxis(axis, layout, shape) ? shape.GetDim(GetAxisIdx(axis, layout)) : kInvalidDimValue;
}

ge::graphStatus QSFAInfoParserMtp::CheckRequiredInOutExistence() const
{
...
ge::graphStatus QSFAInfoParserMtp::CheckRequiredAttrExistence() const
{
...
ge::graphStatus QSFAInfoParserMtp::CheckRequiredParaExistence() const
{
...
ge::graphStatus QSFAInfoParserMtp::GetActualSeqLenQSize(uint32_t &size)
{
    return GetActualSeqLenSize(size, opParamInfo_.actualSeqLengthsQ.tensor, "actualSeqLengthsQ", opName_);
}

ge::graphStatus QSFAInfoParserMtp::GetOpName()
{
    if (context_->GetNodeName() == nullptr) {
        OP_LOGE_WITH_INVALID_INPUT("KvQuantSparseFlashAttentionMtp", "opName");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus QSFAInfoParserMtp::GetNpuInfo()
{
...
void QSFAInfoParserMtp::GetOptionalInputParaInfo()
{
...
void QSFAInfoParserMtp::GetInputParaInfo()
{
...
void QSFAInfoParserMtp::GetOutputParaInfo()
{
...
ge::graphStatus QSFAInfoParserMtp::GetAttrParaInfo()
{
...
ge::graphStatus QSFAInfoParserMtp::GetOpParaInfo()
{
    GetInputParaInfo();
    GetOutputParaInfo();
...
ge::graphStatus QSFAInfoParserMtp::GetInOutDataType()
{
...
ge::graphStatus QSFAInfoParserMtp::GetBatchSize()
{
    // 获取B基准值
    // 1、非TND时, 以query的batch_size维度为基准;
    // 2、TND时, actual_seq_lens_q必须传入, 以actual_seq_lens_q数组的长度为B轴大小
    if (qLayout_ == QSFALayoutMtp::TND) {
        return GetActualSeqLenQSize(bSize_);
    } else { // BSND
        bSize_ = GetAxisNum(queryShape_, QSFAAxisMtp::B, qLayout_);
        return ge::GRAPH_SUCCESS;
    }
}

ge::graphStatus QSFAInfoParserMtp::GetQTSize()
{
    // 获取query的T基准值
    // 1、非TND时, 以query的batch_size维度为基准;
    // 2、TND时, actual_seq_lens_q必须传入, 以actual_seq_lens_q数组的长度为B轴大小
    qTSize_ = (qLayout_ == QSFALayoutMtp::TND) ? GetAxisNum(queryShape_, QSFAAxisMtp::T, qLayout_) : 0;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetKVTSize()
{
    // 获取query的T基准值
    // 1、非TND时, 以key的batch_size维度为基准;
    // 2、TND时, actual_seq_lens_q必须传入, 以actual_seq_lens_q数组的长度为B轴大小
    kvTSize_ = (kvLayout_ == QSFALayoutMtp::TND) ? GetAxisNum(keyShape_, QSFAAxisMtp::T, kvLayout_) : 0;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetQHeadDim()
{
    // 获取qHeadDim基准值
    // 以query的D维度为基准
    qHeadDim_ = GetAxisNum(queryShape_, QSFAAxisMtp::D, qLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetKHeadDim()
{
    // 获取kHeadDim基准值
    // 以key的D维度为基准
    kHeadDim_ = GetAxisNum(keyShape_, QSFAAxisMtp::D, kvLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetS1Size()
{
    // 获取S1基准值
    // 1、非TND时, 以query的S维度为基准;
    // 2、TND时, actual_seq_lens_q必须传入, 以actual_seq_lens_q数组中的最大值为基准
    if (qLayout_ == QSFALayoutMtp::TND) {
        s1Size_ = GetAxisNum(queryShape_, QSFAAxisMtp::T, qLayout_);
        return ge::GRAPH_SUCCESS;
    } else { // BSND
        s1Size_ = GetAxisNum(queryShape_, QSFAAxisMtp::S, qLayout_);
    }
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetKvStorageMode()
{
    if (kvLayout_ == QSFALayoutMtp::PA_BSND) {
        kvStorageMode_ = KvStorageModeMtp::PAGE_ATTENTION;
    } else {
        kvStorageMode_ = KvStorageModeMtp::BATCH_CONTINUOUS;
    }
    // kv存储模式基准值
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetKvLayout()
{
    const map<string, QSFALayoutMtp> layoutKVMap = {
        {"BSND",        QSFALayoutMtp::BSND},
        {"PA_BSND",     QSFALayoutMtp::PA_BSND},
        {"TND",         QSFALayoutMtp::TND}
    };
    ...
    if (kvLayout_ != QSFALayoutMtp::PA_BSND && qLayout_ != kvLayout_) {
        OP_LOGE_FOR_INVALID_FORMATS_WITH_REASON(opName_, "KV and Q",
            QSFALayoutMtpToSerialStringMtp(kvLayout_) + " and " + QSFALayoutMtpToSerialStringMtp(qLayout_),
    ...
    if (kvLayout_ == QSFALayoutMtp::PA_BSND && keyDimNum != 4U) {
...
ge::graphStatus QSFAInfoParserMtp::GetS2SizeForBatchContinuous()
{
    if (kvLayout_ == QSFALayoutMtp::BSND) { // BSND
        s2Size_ = GetAxisNum(keyShape_, QSFAAxisMtp::S, kvLayout_);
    } else if (kvLayout_ == QSFALayoutMtp::TND) { // TND
        s2Size_ = GetAxisNum(keyShape_, QSFAAxisMtp::T, kvLayout_);
    }

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetMaxBlockNumPerBatch()
{
...
ge::graphStatus QSFAInfoParserMtp::GetSparseBlockCount()
{
    sparseBlockCount_ = GetAxisNum(sparseIndicesShape_, QSFAAxisMtp::K, qLayout_);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetBlockSize()
{
    blockSize_ = GetAxisNum(keyShape_, QSFAAxisMtp::Bs, kvLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetS2SizeForPageAttention()
{
...
ge::graphStatus QSFAInfoParserMtp::GetS2Size()
{
    // 获取S2基准值
...
ge::graphStatus QSFAInfoParserMtp::GetValueHeadDim()
{
    // 获取vHeadDim基准值
    // 以value的D维度为基准
    vHeadDim_ = GetAxisNum(valueShape_, QSFAAxisMtp::D, kvLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetDSizeKV()
{
    dSizeKV_ = GetAxisNum(keyShape_, QSFAAxisMtp::D, kvLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetQueryAndOutLayout()
{
    // 获取query和attentionOut的Layout基准值
    // layoutQuery: {qLayout, outLayout}
    const std::map<std::string, std::pair<QSFALayoutMtp, QSFALayoutMtp>> qsfaLayoutMap = {
        {"BSND",        {QSFALayoutMtp::BSND,    QSFALayoutMtp::BSND}},
        {"TND",         {QSFALayoutMtp::TND,     QSFALayoutMtp::TND }},
    };
...
ge::graphStatus QSFAInfoParserMtp::GetTopkLayout()
{
    topkLayout_ = qLayout_;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetN1Size()
{
    n1Size_ = GetAxisNum(queryShape_, QSFAAxisMtp::N, qLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus QSFAInfoParserMtp::GetN2Size()
{
    n2Size_ = GetAxisNum(keyShape_, QSFAAxisMtp::N, kvLayout_);
    return ge::GRAPH_SUCCESS;
}

void QSFAInfoParserMtp::SetQSFAShape()
{
...
ge::graphStatus QSFAInfoParserMtp::GetGSize()
{
    if (n2Size_ != 0) {
...
ge::graphStatus QSFAInfoParserMtp::GetActualseqInfo()
{
...
ge::graphStatus QSFAInfoParserMtp::GetShapeAndSizeInfo()
{
    SetQSFAShape();
...
void QSFAInfoParserMtp::GenerateInfo(QSFATilingInfoMtp &qsfaInfo)
{
    ...
    qsfaInfo.pageAttentionFlag = (kvStorageMode_ == KvStorageModeMtp::PAGE_ATTENTION);
...
void QSFAInfoParserMtp::FillTilingInfoAttrsAndLayouts(QSFATilingInfoMtp &qsfaInfo)
{
...
ge::graphStatus QSFAInfoParserMtp::Parse(QSFATilingInfoMtp &qsfaInfo)
{
    if (context_ == nullptr) {
        OP_LOGE_WITH_INVALID_INPUT("KvQuantSparseFlashAttentionMtp", "tiling context");
        return ge::GRAPH_FAILED;
    }
...
IMPL_OP_OPTILING(A5SparseTailAttentionMtpC8)
    .Tiling(TilingKvQuantSparseFlashAttentionMtp)
    .TilingParse<KvQuantSparseFlashAttentionMtpCompileInfo>(TilingPrepareForKvQuantSparseFlashAttentionMtp);
} // namespace optiling
