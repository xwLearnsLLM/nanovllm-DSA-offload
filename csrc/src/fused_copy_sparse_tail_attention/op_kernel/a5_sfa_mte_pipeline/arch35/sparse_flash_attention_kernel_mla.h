...
#include "../../a5_sfa/arch35/sparse_flash_attention_kvcache.h"
...
public:
    ...
    __aicore__ inline void InitSourceAwareGather(
        ...
        uint32_t futureWorkspaceOffsetBytes);
...
private:
    ...
    __aicore__ inline void ConfigureOffloadRunParam(RunParamStr &runParam);
    ...
    __gm__ uint8_t *userWorkspace = nullptr;
};
    ...
    __gm__ uint8_t *gmTiling, TPipe *tPipe)
{
    ...
    cacheTokensGm.SetGlobalBuffer(cacheTokensAddr);
...
SparseFlashAttentionKernelMla<CubeBlockType, VecBlockType>::InitSourceAwareGather(
    __gm__ uint8_t *dramKeyRope, __gm__ uint8_t *dramKey,
    __gm__ uint8_t *dramBlockTable, __gm__ uint8_t *sourceTokenIds,
    __gm__ uint8_t *copyCounts, uint32_t copyCap,
    uint32_t dramMaxBlockNum, uint32_t prefetchRowsPerStep,
    ...
    uint32_t futureWorkspaceOffsetBytes)
{
    constexpr uint32_t tileRows = 128U;
    constexpr uint32_t rowElements = 576U;
    const uint64_t tileBytes = static_cast<uint64_t>(tileRows) *
        rowElements * sizeof(Q_T);
    uint32_t workspaceOwner = static_cast<uint32_t>(aicIdx);
    if constexpr (IS_SPLIT_G) {
        workspaceOwner >>= 1U;
    }
    __gm__ uint8_t *futureWorkspace = userWorkspace +
        futureWorkspaceOffsetBytes +
        static_cast<uint64_t>(workspaceOwner) *
            futureWorkspaceTileCount * tileBytes;
    if ASCEND_IS_AIV {
        vecBlock.InitSourceAwareGather(
            dramKeyRope, dramKey, dramBlockTable, sourceTokenIds,
            copyCounts, copyCap, dramMaxBlockNum, prefetchRowsPerStep,
            futureWorkspace,
            futureWorkspaceMaxMiss, futureWorkspaceTileCount);
    } else {
        cubeBlock.InitFutureWorkspace(
            futureWorkspace, copyCounts, futureWorkspaceMaxMiss,
            futureWorkspaceTileCount);
    }
}

template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void
SparseFlashAttentionKernelMla<CubeBlockType, VecBlockType>::ConfigureOffloadRunParam(
    RunParamStr &runParam)
{
    constexpr int32_t sparseComputeCount = 2048;
    int32_t cacheTokenCount = cacheTokensAddr[runParam.boIdx];
    runParam.cacheTokenCount = cacheTokenCount;
    runParam.sparseTokenCount = 0;
    runParam.tailTokenCount = 0;
    runParam.tailSlotStart = 0;
    runParam.virtualS2Size = 0;

    if (cacheTokenCount == 0) {
        runParam.tailTokenCount = runParam.actualS2Size;
        runParam.virtualS2Size = runParam.actualS2Size;
    } else if (cacheTokenCount >= sparseComputeCount &&
               cacheTokenCount <= runParam.actualS2Size) {
        runParam.sparseTokenCount = sparseComputeCount;
        runParam.tailSlotStart = cacheTokenCount;
        runParam.tailTokenCount = runParam.actualS2Size - cacheTokenCount;
        runParam.virtualS2Size =
            runParam.sparseTokenCount + runParam.tailTokenCount;
    }
    runParam.nextTokensPerBatch =
        runParam.virtualS2Size - runParam.actualS1Size;
}
...
void SparseFlashAttentionKernelMla<CubeBlockType, VecBlockType>::InitCalcParamsEach()
{
    ...
    for (uint32_t bIdx = 0; bIdx < constInfo.bSize; bIdx++) {
        ...
        if constexpr (IS_SPLIT_G) {
            RunParamStr lengthParam;
            lengthParam.boIdx = bIdx;
            lengthParam.n2oIdx = 0;
            ComputeParamBatch<TEMPLATE_INTF_ARGS>(
                lengthParam, this->constInfo,
                this->actualSeqQlenAddr, this->actualSeqKvlenAddr);
            ConfigureOffloadRunParam(lengthParam);
            uint32_t virtualS2Size = lengthParam.virtualS2Size > 0
                ? static_cast<uint32_t>(lengthParam.virtualS2Size)
                : 0;
            if (virtualS2Size > maxVirtualS2Size) {
                maxVirtualS2Size = virtualS2Size;
            }
        }
    }
    ...
    if constexpr (IS_SPLIT_G) {
        // The original SFA synchronizes split-G core pairs for exactly top-K
        // tokens. sparse-and-tail attention can add another partial/full
        // loop, so idle pairs must use the real virtual S2 upper bound too.
        uint32_t maxS2LoopPerBase =
            (maxVirtualS2Size + constInfo.s2BaseSize - 1) /
            constInfo.s2BaseSize;
        maxS2LoopCnt = avgBaseNum * maxS2LoopPerBase;
    }
...
__aicore__ inline void SparseFlashAttentionKernelMla<CubeBlockType, VecBlockType>::ProcessMainLoop()
{
    ...
    for (int64_t bnIdx = bN2StartIdx; bnIdx < bN2EndIdx; bnIdx++) {
        ...
        ComputeParamBatch<TEMPLATE_INTF_ARGS>(runParam, this->constInfo,
            this->actualSeqQlenAddr, this->actualSeqKvlenAddr);
        ConfigureOffloadRunParam(runParam);
        ...
        for (int64_t gS1Index = runParam.gs1LoopStartIdx; gS1Index < gS1LoopEnd; gS1Index++) {
            ...
            for (int64_t s2LoopCount = 0; s2LoopCount <= s2LoopLimit; ++s2LoopCount) {
                ...
                if (notLastTwoLoop) {
                    if ASCEND_IS_AIV {
                        RunInfo &prefetchRunInfo = runInfo[taskId % 3];
                        this->vecBlock.ProcessMtePrefetch(
                            prefetchRunInfo, this->constInfo);
                    }
                }
                if (taskId > 0 && notLast) {
