...
// =====================================================================================
// C8 收尾: Σ 合并 + kScale 乘法 + bf16-sortable（官方 QuantLI ReduceSumFinalize 语义）
// 与官方同形态放在 __simd_callee__ 中，由 6 参 MulWeightAndReduceSum vf 调用。
// kScale[0..64) / kScale[64..128) 分别作用于 S2 列的前后两半（Σ 之后乘）。
// =====================================================================================
__simd_callee__ inline void MulWeightReduceSumFinalizeC8(
...
// float in uint16 out（C8 专用：kScale 必填，Σ 之后乘——官方 QuantLI 语义）
// vf 体内不做数据依赖分支（kScale 无条件加载，与官方 vf 的调用形态一致）。
                                              ...
                                              __ubuf__ float* kScale_)
{
    ...
    AscendC::MicroAPI::RegTensor<float> regSum0[2];
    AscendC::MicroAPI::RegTensor<float> regSum1[2];
    // kScale 寄存器声明在函数顶层，与官方 QuantLI 的 ReduceSumFinalize 声明模式一致
    AscendC::MicroAPI::RegTensor<float> regKScale[2];
    AscendC::MicroAPI::MaskReg maskAllB32 =
    ...
    MulWeightReduceSumFinalizeC8(regSum0, regSum1, regKScale, maskAllB32, bf16Ctx, maskAllB16, out_);
}
