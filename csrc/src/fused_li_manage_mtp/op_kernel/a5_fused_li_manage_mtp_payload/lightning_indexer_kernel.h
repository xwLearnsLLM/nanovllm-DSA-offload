...
public:
    ...
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *weights,
                                ...
                                __gm__ uint8_t *cacheSlots, __gm__ uint8_t *topkSlots,
                                __gm__ uint8_t *missIndex, __gm__ uint8_t *missSlots,
                                __gm__ uint8_t *missCount,
                                __gm__ uint8_t *workspace, const LIMtpA5TilingData *__restrict tiling, TPipe *tPipe);
...
protected:
    ...
    // ================================Init functions==================================
    __aicore__ inline void InitTilingData(const LIMtpA5TilingData *__restrict tilingData);
...
template <typename LIT>
__aicore__ inline void LightningIndexerKernel<LIT>::InitTilingData(const LIMtpA5TilingData *__restrict tilingData)
{
...
__aicore__ inline void LightningIndexerKernel<LIT>::Init(__gm__ uint8_t *query,
                                              ...
                                              __gm__ uint8_t *missIndex, __gm__ uint8_t *missSlots,
                                              __gm__ uint8_t *missCount,
                                              __gm__ uint8_t *workspace,  const LIMtpA5TilingData *__restrict tiling,
                                              TPipe *tPipe)
{
    if ASCEND_IS_AIV {
    ...
    if ASCEND_IS_AIV {
        ...
        missIndexGm.SetGlobalBuffer((__gm__ int32_t *)missIndex);
        missSlotsGm.SetGlobalBuffer((__gm__ int32_t *)missSlots);
        ...
        vectorService.InitVecInputTensor(weightsGm, indiceOutGm, valueOutGm, blockTableGm,
                                         cacheSlotsGm, topkSlotsGm, missIndexGm,
                                         missSlotsGm, missCountGm);
...
__aicore__ inline void LightningIndexerKernel<LIT>::Process()
{
    ...
    if ASCEND_IS_AIV {
        AscendC::SyncAll();
        if ((tmpBlockIdx & 1U) == 0U && aiCoreIdx < usedCoreNum) {
            uint32_t requestWorkerCount = usedCoreNum;
            for (uint32_t bIdx = aiCoreIdx; bIdx < constInfo.batchSize;
                 bIdx += requestWorkerCount) {
                uint32_t queryEnd = actualSeqLengthsGmQ.GetValue(bIdx);
                uint32_t queryBegin = bIdx == 0U ? 0U : actualSeqLengthsGmQ.GetValue(bIdx - 1U);
                uint32_t actualKeyLen = actualSeqLengthsGmKv.GetValue(bIdx);
                vectorService.FinalizeMtpRequest(
                    bIdx, queryBegin, queryEnd - queryBegin, actualKeyLen);
            }
        }
    }
}
