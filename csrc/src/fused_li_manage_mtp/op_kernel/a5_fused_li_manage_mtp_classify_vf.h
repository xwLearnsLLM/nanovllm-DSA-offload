...
#ifndef LIGHTNING_INDEXER_DECODE_UPDATE_MTP_A5_CLASSIFY_VF_H
#define LIGHTNING_INDEXER_DECODE_UPDATE_MTP_A5_CLASSIFY_VF_H
...
__simd_vf__ void DecodeTokenIds(__ubuf__ uint32_t *payload, uint32_t vecLoopNum)
{
    ...
    for (uint32_t i = 0; i < vecLoopNum; ++i) {
        ...
        MicroAPI::StoreAlign<uint32_t, MicroAPI::StoreDist::DIST_NORM>(
            payload + i * CHUNK_SIZE, token, all);
    }
