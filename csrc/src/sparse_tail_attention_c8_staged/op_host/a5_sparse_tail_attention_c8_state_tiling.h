...
constexpr uint32_t STAGE_MODE_ATTR_INDEX = 13;
constexpr uint32_t PROBE_ENABLED_ATTR_INDEX = 14;
// Dim Num
...
struct QSFAParaInfo {
    ...
    const int64_t *stageMode = nullptr;
    const bool *probeEnabled = nullptr;
};
...
TILING_DATA_FIELD_DEF(uint32_t, stageMode)
END_TILING_DATA_DEF
...
END_TILING_DATA_DEF
#if defined(A5_C8_PML_PROBE_TILING_ONLY)
REGISTER_TILING_DATA_CLASS(A5SparseTailAttentionC8PmlProbe, KvQuantSparseFlashAttentionTilingDataMla)
