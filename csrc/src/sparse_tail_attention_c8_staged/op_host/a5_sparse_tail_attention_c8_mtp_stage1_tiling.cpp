...
string QSFADebugTilingContext(const gert::TilingContext *context)
{
    ...
    const auto *nodeInfo = context->GetComputeNodeInfo();
    if (nodeInfo == nullptr) {
        return "compute_node_info=null";
    }
    std::ostringstream qsfaOss;
...
void QSFAMlaTiling::GenTilingKey()
{
    ...
    tilingKey_ = GET_TPL_TILING_KEY(0U, pageAttention, layoutQuery, layoutKV,
        ...
        static_cast<uint32_t>(qsfaInfo_->gSize > 64),
        static_cast<uint32_t>(qsfaInfo_->probeEnabled)); // G大于64时核间切G
...
void QSFAMlaTiling::FillTilingBaseParamsMla()
{
    ...
    tilingData_.baseParams.set_stageMode(qsfaInfo_->stageMode);
}
...
ge::graphStatus QSFAMlaTiling::DoOpTiling(QSFATilingInfo *qsfaInfo)
{
    ...
    if (GetPlatformInfo() != ge::GRAPH_SUCCESS) {
        OP_LOGE(qsfaInfo_->opName,
            "[C8_STAGED_TILING] failed at DoOpTiling.GetPlatformInfo");
        return ge::GRAPH_FAILED;
    }
    InitParams();
    Split();
    ...
    if (SetBlockDim(blockDim_) != ge::GRAPH_SUCCESS) {
        OP_LOGE(qsfaInfo_->opName,
            "[C8_STAGED_TILING] failed at DoOpTiling.SetBlockDim, block_dim=%u",
            blockDim_);
        return ge::GRAPH_FAILED;
    }
    if (SetTilingKey(tilingKey_) != ge::GRAPH_SUCCESS) {
        OP_LOGE(qsfaInfo_->opName,
            "[C8_STAGED_TILING] failed at DoOpTiling.SetTilingKey, tiling_key=%lu",
            tilingKey_);
        return ge::GRAPH_FAILED;
    }
    if (SetWorkspaceSize(workspaceSize_) != ge::GRAPH_SUCCESS) {
        OP_LOGE(qsfaInfo_->opName,
            "[C8_STAGED_TILING] failed at DoOpTiling.SetWorkspaceSize, workspace=%zu",
            workspaceSize_);
        return ge::GRAPH_FAILED;
    }
    if (SetTilingData(tilingData_) != ge::GRAPH_SUCCESS) {
        OP_LOGE(qsfaInfo_->opName,
            "[C8_STAGED_TILING] failed at DoOpTiling.SetTilingData");
        return ge::GRAPH_FAILED;
    }

    OP_LOGI(qsfaInfo_->opName,
        "[C8_STAGED_TILING] success: stage=%u B=%u S1=%u S2=%u N1=%u N2=%u "
        "sparse_count=%u block_dim=%u workspace=%zu tiling_key=%lu",
        qsfaInfo_->stageMode, qsfaInfo_->bSize, qsfaInfo_->s1Size,
        qsfaInfo_->s2Size, qsfaInfo_->n1Size, qsfaInfo_->n2Size,
        qsfaInfo_->sparseBlockCount, blockDim_, workspaceSize_, tilingKey_);
    return ge::GRAPH_SUCCESS;
}
...
{
    if (context == nullptr) {
        OP_LOGE("A5SparseTailAttentionC8",
            "[C8_STAGED_TILING] entry received a null context");
        return ge::GRAPH_FAILED;
    }
    const char *nodeName = context->GetNodeName();
    const std::string contextDump = QSFADebugTilingContext(context);
    OP_LOGI(nodeName == nullptr ? "A5SparseTailAttentionC8" : nodeName,
        "[C8_STAGED_TILING] entry node=%s %s",
        nodeName == nullptr ? "<null>" : nodeName, contextDump.c_str());

    QSFATilingInfo qsfaInfo;
    QSFAInfoParser qsfaInfoParser(context);
    if (qsfaInfoParser.Parse(qsfaInfo) != ge::GRAPH_SUCCESS) {
        OP_LOGE(nodeName == nullptr ? "A5SparseTailAttentionC8" : nodeName,
            "[C8_STAGED_TILING] failed at QSFAInfoParser.Parse");
        return ge::GRAPH_FAILED;
    }
    OP_LOGI(qsfaInfo.opName,
        "[C8_STAGED_TILING] parsed: stage=%u B=%u S1=%u S2=%u N1=%u N2=%u "
        "Q_D=%u K_D=%u V_D=%u sparse_count=%u block_size=%ld max_blocks=%u",
        qsfaInfo.stageMode, qsfaInfo.bSize, qsfaInfo.s1Size,
        qsfaInfo.s2Size, qsfaInfo.n1Size, qsfaInfo.n2Size,
        qsfaInfo.qHeadDim, qsfaInfo.kHeadDim, qsfaInfo.vHeadDim,
        qsfaInfo.sparseBlockCount, qsfaInfo.blockSize,
        qsfaInfo.maxBlockNumPerBatch);

    QSFATilingCheck tilingChecker(qsfaInfo);
    if (tilingChecker.Process() != ge::GRAPH_SUCCESS) {
        OP_LOGE(qsfaInfo.opName,
            "[C8_STAGED_TILING] failed at QSFATilingCheck.Process");
        return ge::GRAPH_FAILED;
    }
    OP_LOGI(qsfaInfo.opName,
        "[C8_STAGED_TILING] validation passed");

    QSFAMlaTiling tiling(context);
    const ge::graphStatus status = tiling.DoOpTiling(&qsfaInfo);
    if (status != ge::GRAPH_SUCCESS) {
        OP_LOGE(qsfaInfo.opName,
            "[C8_STAGED_TILING] failed at QSFAMlaTiling.DoOpTiling");
    }
    return status;
}
...
ge::graphStatus QSFATilingCheck::CheckSinglePara() const
{
    if (ge::GRAPH_SUCCESS != CheckSingleParaQuery()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckSinglePara.Query failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckSingleParaKey()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckSinglePara.Key failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckSingleParaSparseIndices()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckSinglePara.SparseIndices failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckSingleParaNumHeads()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckSinglePara.NumHeads failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckSingleParaKvHeadNums()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckSinglePara.KvHeadNums failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckSingleParaSparseMode()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckSinglePara.SparseMode failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckSingleParaSparseBlockSize()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckSinglePara.SparseBlockSize failed");
        return ge::GRAPH_FAILED;
    }
...
{
    constexpr int64_t STAGED_TOPK_COUNT = 2048;
    QSFATilingShapeCompareParam qsfaShapeParams;
    qsfaShapeParams.B = bSize_;
    qsfaShapeParams.N = n2Size_;
    qsfaShapeParams.S = s1Size_;
    if (qsfaInfo_.stageMode <= 2 ||
        (qsfaInfo_.stageMode == 4 && qsfaInfo_.probeEnabled)) {
        qsfaShapeParams.D = STAGED_TOPK_COUNT;
    } else {
        qsfaShapeParams.D =
            topkShapeCmp_.GetDim(topkShapeCmp_.GetDimNum() - 1);
    }
...
{
    const ge::DataType actualDtype = opParamInfo_.attenOut.desc->GetDataType();
    constexpr uint32_t STAGE_ONE = 1;
    const ge::DataType expectedDtype = qsfaInfo_.stageMode == STAGE_ONE
        ? ge::DT_FLOAT
        : inputQType_;
    if (ge::GRAPH_SUCCESS != CheckDTypeConsistency(
        actualDtype, expectedDtype, ATTEN_OUT_NAME)) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] CheckAttenOut.Dtype failed: stage=%u actual=%d expected=%d",
            qsfaInfo_.stageMode, static_cast<int32_t>(actualDtype),
            static_cast<int32_t>(expectedDtype));
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckAttenOutShape()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] CheckAttenOut.Shape failed: stage=%u",
            qsfaInfo_.stageMode);
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
...
ge::graphStatus QSFATilingCheck::CheckMultiParaConsistency()
{
    ...
    if (ge::GRAPH_SUCCESS != CheckKV()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] consistency failed at CheckKV");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckTopK()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] consistency failed at CheckTopK");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckAttenOut()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] consistency failed at CheckAttenOut");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckActualSeqLensQ()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] consistency failed at CheckActualSeqLensQ");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckActualSeqLens()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] consistency failed at CheckActualSeqLens");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckBlockTable()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] consistency failed at CheckBlockTable");
        return ge::GRAPH_FAILED;
    }
...
{
    if (isA5_) {
        // Staged Stage1 appends the dense tail after the 2048 LI slots.  The
        // kernel stops at the first -1, so a wider K remains ABI-compatible.
...
ge::graphStatus QSFATilingCheck::CheckFeatureMlaAntiquant() const
{
    if (ge::GRAPH_SUCCESS != CheckFeatureMlaAntiquantAttr()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckFeature.Attr failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckFeatureMlaAntiquantShape()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckFeature.Shape failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckFeatureMlaAntiquantLayout()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckFeature.Layout failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckFeatureMlaAntiquantDtype()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckFeature.Dtype failed");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckFeatureMlaAntiquantPa()) {
        OP_LOGE(opName_, "[C8_STAGED_TILING] CheckFeature.PageAttention failed");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus QSFATilingCheck::Process()
{
    ...
    if (CheckSinglePara() != ge::GRAPH_SUCCESS) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] validation failed at CheckSinglePara");
        return ge::GRAPH_FAILED;
    }
    if (CheckParaExistence() != ge::GRAPH_SUCCESS) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] validation failed at CheckParaExistence");
        return ge::GRAPH_FAILED;
    }
    if (CheckFeature() != ge::GRAPH_SUCCESS) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] validation failed at CheckFeature");
        return ge::GRAPH_FAILED;
    }
    if (CheckMultiParaConsistency() != ge::GRAPH_SUCCESS) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] validation failed at CheckMultiParaConsistency");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus QSFAInfoParser::GetAttrParaInfo()
{
    ...
    opParamInfo_.stageMode = attrs->GetAttrPointer<int64_t>(STAGE_MODE_ATTR_INDEX);
    if (opParamInfo_.stageMode != nullptr && *opParamInfo_.stageMode >= 3) {
        opParamInfo_.probeEnabled =
            attrs->GetAttrPointer<bool>(PROBE_ENABLED_ATTR_INDEX);
    }
...
{
    constexpr uint32_t STAGE_ONE = 1;
    constexpr uint32_t TND_PROBE = 4;
    sparseBlockCount_ = GetAxisNum(sparseIndicesShape_, QSFAAxis::K, qLayout_);
    if (opParamInfo_.stageMode != nullptr &&
        (*opParamInfo_.stageMode == STAGE_ONE ||
         (*opParamInfo_.stageMode == TND_PROBE &&
          opParamInfo_.probeEnabled != nullptr &&
          *opParamInfo_.probeEnabled))) {
        sparseBlockCount_ = std::max<uint32_t>(sparseBlockCount_, s2Size_);
    }
...
void QSFAInfoParser::FillTilingInfoAttrsAndLayouts(QSFATilingInfo &qsfaInfo)
{
    ...
    qsfaInfo.stageMode = static_cast<uint32_t>(*opParamInfo_.stageMode);
    qsfaInfo.probeEnabled = opParamInfo_.probeEnabled != nullptr &&
        *opParamInfo_.probeEnabled;
...
ge::graphStatus QSFAInfoParser::Parse(QSFATilingInfo &qsfaInfo)
{
    ...
    if (ge::GRAPH_SUCCESS != GetOpName()) {
        OP_LOGE("KvQuantSparseFlashAttention",
            "[C8_STAGED_TILING] parser failed at GetOpName");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != GetNpuInfo()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at GetNpuInfo");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != GetOpParaInfo()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at GetOpParaInfo");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != CheckRequiredParaExistence()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at CheckRequiredParaExistence");
        return ge::GRAPH_FAILED;
    }

    if (ge::GRAPH_SUCCESS != GetInOutDataType()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at GetInOutDataType");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != GetQueryAndOutLayout()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at GetQueryAndOutLayout");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != GetTopkLayout()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at GetTopkLayout");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != GetKvLayout()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at GetKvLayout");
        return ge::GRAPH_FAILED;
    }
    if (ge::GRAPH_SUCCESS != GetKvStorageMode()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at GetKvStorageMode");
        return ge::GRAPH_FAILED;
    }

    if (ge::GRAPH_SUCCESS != GetShapeAndSizeInfo()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at GetShapeAndSizeInfo");
        return ge::GRAPH_FAILED;
    }

    if (ge::GRAPH_SUCCESS != GetActualseqInfo()) {
        OP_LOGE(opName_,
            "[C8_STAGED_TILING] parser failed at GetActualseqInfo");
        return ge::GRAPH_FAILED;
    }
