...
namespace BaseApi {
enum C8ProbeMode : int {
    C8_PROBE_DISABLED = 0,
    C8_PROBE_BASELINE = 1,
    C8_PROBE_PML_WRITE = 2,
    C8_PROBE_TND_RESOLVE = 3,
};

// Staged sparse slots are laid out as one compact hit segment followed by
// one dense tail segment.  Select the segment once per Vector S2 sub-tile so
// the hot two-token gather loop does not branch for every token.
enum C8SlotRegion : int {
    C8_SLOT_GENERIC = 0,
    C8_SLOT_DENSE = 1,
    C8_SLOT_TOPK = 2,
    C8_SLOT_HIT = 3,
