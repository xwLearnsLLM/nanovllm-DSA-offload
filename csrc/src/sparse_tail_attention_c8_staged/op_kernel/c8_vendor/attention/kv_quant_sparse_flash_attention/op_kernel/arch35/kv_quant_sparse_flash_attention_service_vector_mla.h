...
public:
    ...
    template <bool ENABLE_FINAL_MERGE, int PROBE_MODE = C8_PROBE_DISABLED>
    __aicore__ inline void InitStageOutputs(
        __gm__ uint8_t *missCounts, __gm__ uint8_t *cacheTokens,
...
private:
    ...
    __aicore__ inline void ProcessSparseKv(Buffer<BufferType::L1, SyncType::CROSS_CORE_SYNC_FORWARD> &outputL1,
        Buffer<BufferType::GM, SyncType::CROSS_CORE_SYNC_BACKWARD> &v0ResGm,
        const RunInfo &runInfo, ConstInfo &constInfo);
    template <bool ENABLE_FINAL_MERGE, int PROBE_MODE, int SLOT_REGION>
    __aicore__ inline void ProcessSparseKvRegion(Buffer<BufferType::L1, SyncType::CROSS_CORE_SYNC_FORWARD> &outputL1,
        Buffer<BufferType::GM, SyncType::CROSS_CORE_SYNC_BACKWARD> &v0ResGm,
        const RunInfo &runInfo, ConstInfo &constInfo);
    ...
    template <bool ENABLE_FINAL_MERGE, int PROBE_MODE = C8_PROBE_DISABLED,
        int SLOT_REGION = C8_SLOT_GENERIC>
    __aicore__ inline void GetRealCmpS2Idx(int64_t &token0Idx, int64_t &token1Idx, int64_t s2IdxInBase,
        const RunInfo &runInfo, ConstInfo &constInfo);
    template <bool ENABLE_FINAL_MERGE, int PROBE_MODE = C8_PROBE_DISABLED,
        int SLOT_REGION = C8_SLOT_GENERIC>
    __aicore__ inline int64_t GetLogicalTokenIdx(
        int64_t logicalIdx, const RunInfo &runInfo);
    ...
    TBuf<> commonTBuf; // common的复用空间，Stage2 merge复用其中512个FP32元素
    ...
    TQue<QuePosition::VECIN, 2> previousPQue; // Stage2 merge预取previous P, pingpong隐藏MTE2延迟
    TBuf<> stage2OutBuf;
    TEventID mte3ToVId[2]; // 存放MTE3_V的eventId, 2份表示可能存在pingpong
    TEventID vToMte3Id[2]; // 存放V_MTE3的eventId, 2份表示可能存在pingpong
    TEventID mte2ToVId; // Stage2 merge批量读取previous M/L
...
{
    int64_t probeSlotRowOffset = 0;
    if constexpr (PROBE_MODE == C8_PROBE_BASELINE ||
                  PROBE_MODE == C8_PROBE_PML_WRITE) {
        if constexpr (LAYOUT_T == QSFA_LAYOUT::TND) {
            const uint64_t queryPrefix = runInfo.boIdx == 0
                ? 0
                : cuSeqlensQGm.GetValue(runInfo.boIdx - 1);
            probeSlotRowOffset =
                (queryPrefix + runInfo.s1oIdx) *
                constInfo.sparseBlockCount;
        } else {
            probeSlotRowOffset =
                (runInfo.boIdx * constInfo.s1Size + runInfo.s1oIdx) *
                constInfo.sparseBlockCount;
        }
    }
    ...
    if (unlikely(qsfaTopkIdx >= constInfo.sparseBlockCount)) {
        token0Idx = -1;
    } else if constexpr (PROBE_MODE == C8_PROBE_BASELINE ||
                         PROBE_MODE == C8_PROBE_PML_WRITE) {
        token0Idx = SparseIndicesGm.GetValue(
            probeSlotRowOffset + qsfaTopkIdx) + runInfo.s2StartIdx;
    } else {
        token0Idx = GetLogicalTokenIdx<ENABLE_FINAL_MERGE, PROBE_MODE, SLOT_REGION>(
            qsfaTopkIdx, runInfo);
    }
    qsfaTopkIdx += 1;
    if (unlikely((qsfaTopkIdx >= constInfo.sparseBlockCount) || (s2IdxInBase + 1 >= sparseS2End))) {
        ...
        token1Idx = GetLogicalTokenIdx<ENABLE_FINAL_MERGE, PROBE_MODE, SLOT_REGION>(
            qsfaTopkIdx, runInfo);
    }
}
...
QSFAVectorService<TEMPLATE_ARGS>::GetLogicalTokenIdx(
    int64_t logicalIdx, const RunInfo &runInfo)
{
    if constexpr (SLOT_REGION == C8_SLOT_DENSE) {
        return logicalIdx + runInfo.s2StartIdx;
    } else if constexpr (SLOT_REGION == C8_SLOT_TOPK) {
        return SparseIndicesGm.GetValue(
            runInfo.stagedTopkRowOffset + logicalIdx) +
            runInfo.s2StartIdx;
    } else if constexpr (SLOT_REGION == C8_SLOT_HIT) {
        const int64_t hitOffset = PROBE_MODE == C8_PROBE_TND_RESOLVE
            ? logicalIdx
            : runInfo.stagedMissCount + logicalIdx;
        return SparseIndicesGm.GetValue(
            runInfo.stagedTopkRowOffset + hitOffset) +
            runInfo.s2StartIdx;
    } else if constexpr (SLOT_REGION == C8_SLOT_TAIL) {
        return runInfo.stagedCacheTokens + logicalIdx -
            runInfo.stagedHitCount + runInfo.s2StartIdx;
    } else if constexpr (SLOT_REGION == C8_SLOT_EMPTY) {
        return runInfo.s2StartIdx;
    } else if constexpr (PROBE_MODE == C8_PROBE_TND_RESOLVE) {
        if (runInfo.stagedCacheTokens == 0) {
            return logicalIdx + runInfo.s2StartIdx;
        }
        if (logicalIdx < runInfo.stagedHitCount) {
            return SparseIndicesGm.GetValue(
                runInfo.stagedTopkRowOffset + logicalIdx) +
                runInfo.s2StartIdx;
        }
        return runInfo.stagedCacheTokens + logicalIdx -
            runInfo.stagedHitCount + runInfo.s2StartIdx;
    } else if constexpr (ENABLE_FINAL_MERGE) {
        return SparseIndicesGm.GetValue(
            runInfo.stagedTopkRowOffset + logicalIdx) +
            runInfo.s2StartIdx;
    } else {
        ...
        if (logicalIdx < runInfo.stagedHitCount) {
            return SparseIndicesGm.GetValue(
                runInfo.stagedTopkRowOffset + runInfo.stagedMissCount +
                logicalIdx) + runInfo.s2StartIdx;
        }
        return runInfo.stagedCacheTokens + logicalIdx -
            runInfo.stagedHitCount + runInfo.s2StartIdx;
    }
...
__aicore__ inline void QSFAVectorService<TEMPLATE_ARGS>::CalSparseCalSize(const RunInfo &runInfo, ConstInfo &constInfo)
{
    if constexpr (IS_SPLIT_G) {
        ...
        int64_t splitCandidate = s2Loops * s2PerVecLoop;
        int64_t splitPoint = splitCandidate < runInfo.s2RealSize
            ? splitCandidate
            : runInfo.s2RealSize;
        sparseS2Start = GetSubBlockIdx() == 0 ? 0 : splitPoint;
        sparseS2End = GetSubBlockIdx() == 0 ? splitPoint : runInfo.s2RealSize;
        qsfaSparseCalSize = sparseS2End - sparseS2Start;
    }
}

TEMPLATES_DEF_NO_DEFAULT
template <bool ENABLE_FINAL_MERGE, int PROBE_MODE>
    ...
    const RunInfo &runInfo, ConstInfo &constInfo)
{
    ...
    ProcessSparseKv<ENABLE_FINAL_MERGE, PROBE_MODE>(
        outputL1, v0ResGm, runInfo, constInfo);
    ...
    Buffer<BufferType::GM, SyncType::CROSS_CORE_SYNC_BACKWARD> &v0ResGm, const RunInfo &runInfo, ConstInfo &constInfo)
{
    if constexpr (PROBE_MODE == C8_PROBE_BASELINE ||
                  PROBE_MODE == C8_PROBE_PML_WRITE) {
        ProcessSparseKvRegion<ENABLE_FINAL_MERGE, PROBE_MODE, C8_SLOT_GENERIC>(
            outputL1, v0ResGm, runInfo, constInfo);
    } else if constexpr (ENABLE_FINAL_MERGE) {
        ProcessSparseKvRegion<ENABLE_FINAL_MERGE, PROBE_MODE, C8_SLOT_TOPK>(
            outputL1, v0ResGm, runInfo, constInfo);
    } else if (unlikely(runInfo.stagedSlotSetEmpty)) {
        ProcessSparseKvRegion<ENABLE_FINAL_MERGE, PROBE_MODE, C8_SLOT_EMPTY>(
            outputL1, v0ResGm, runInfo, constInfo);
    } else if (runInfo.stagedCacheTokens == 0) {
        ProcessSparseKvRegion<ENABLE_FINAL_MERGE, PROBE_MODE, C8_SLOT_DENSE>(
            outputL1, v0ResGm, runInfo, constInfo);
    } else {
        // Classify the Vector sub-tile, not merely the AIC S2 tile. Usually
        // every worker is wholly hit or wholly tail; only the worker that
        // owns the single boundary needs the mixed per-token comparison.
        const int64_t logicalBase =
            runInfo.s2LoopCount * constInfo.s2BaseSize;
        const int64_t logicalStart = logicalBase + sparseS2Start;
        const int64_t logicalEnd = logicalBase + sparseS2End;
        if (logicalEnd <= runInfo.stagedHitCount) {
            ProcessSparseKvRegion<ENABLE_FINAL_MERGE, PROBE_MODE, C8_SLOT_HIT>(
                outputL1, v0ResGm, runInfo, constInfo);
        } else if (logicalStart >= runInfo.stagedHitCount) {
            ProcessSparseKvRegion<ENABLE_FINAL_MERGE, PROBE_MODE, C8_SLOT_TAIL>(
                outputL1, v0ResGm, runInfo, constInfo);
        } else {
            ProcessSparseKvRegion<ENABLE_FINAL_MERGE, PROBE_MODE, C8_SLOT_MIXED>(
                outputL1, v0ResGm, runInfo, constInfo);
        }
    }
}
...
TEMPLATES_DEF_NO_DEFAULT
template <bool ENABLE_FINAL_MERGE, int PROBE_MODE, int SLOT_REGION>
__aicore__ inline void QSFAVectorService<TEMPLATE_ARGS>::ProcessSparseKvRegion(
    Buffer<BufferType::L1, SyncType::CROSS_CORE_SYNC_FORWARD> &outputL1,
    Buffer<BufferType::GM, SyncType::CROSS_CORE_SYNC_BACKWARD> &v0ResGm, const RunInfo &runInfo, ConstInfo &constInfo)
{
    ...
    while ((s2 < sparseS2End) && !meetEnd) { // 拷贝到s2End或者遇到-1
        ...
        while (dealRow < Min(16, qsfaSparseCalSize) && s2<sparseS2End) { // 拷贝满16行或者遇到-1
            GetRealCmpS2Idx<ENABLE_FINAL_MERGE, PROBE_MODE, SLOT_REGION>(
    ...
    ConstInfo &constInfo)
{
    ...
    if (unlikely(runInfo.s2LoopCount == 0)) {
        ...
        if constexpr (PROBE_MODE == C8_PROBE_DISABLED) {
            // Keep the staged P state as an unnormalized numerator.  The
            // merge kernel combines P/M/L and performs final normalization.
            ...
            FlashUpdateLastNew<T, Q_T, OUTPUT_T, qsfaDTemplateAlign64, false, false>(
                vec2ResUb, mmRes, vec2ResUb, qsfaExpUb, qsfaExpUb,
                sumUb, runInfo.vec2MRealSize, qsfaDTemplateAlign64, 1.0, 1.0);
        }
    }
    ...
    if (runInfo.s2LoopCount == runInfo.s2LoopLimit) {
        if constexpr (PROBE_MODE != C8_PROBE_DISABLED) {
            ...
            if constexpr (PROBE_MODE == C8_PROBE_PML_WRITE) {
                LocalTensor<float> maxUb =
                    this->softmaxMaxBuf[runInfo.multiCoreIdxMod2].template Get<float>();
                // Probe values are intentionally ignored. Copying after the
                // in-place output cast preserves the real P/M/L GM traffic
                // without perturbing the upstream attention finalization.
                this->CopyOutState(
        ...
        LocalTensor<float> sumUb = this->softmaxSumBuf[runInfo.multiCoreIdxMod2].template Get<float>();
        LocalTensor<float> maxUb = this->softmaxMaxBuf[runInfo.multiCoreIdxMod2].template Get<float>();
        if constexpr (ENABLE_FINAL_MERGE) {
            if (unlikely(runInfo.stagedMissCount == 0)) {
                Duplicate(vec2ResUb, 0.0f, qsfaVec2CalcSize);
                Duplicate(sumUb, 0.0f, runInfo.vec2MRealSize);
                LocalTensor<uint32_t> maxBits = maxUb.template ReinterpretCast<uint32_t>();
        ...
        if constexpr (ENABLE_FINAL_MERGE) {
            ...
            if (unlikely(runInfo.stagedValidCount == 0)) {
                Duplicate(vec2ResUb, 0.0f, qsfaVec2CalcSize);
                Duplicate(sumUb, 0.0f, runInfo.vec2MRealSize);
                LocalTensor<uint32_t> maxBits = maxUb.template ReinterpretCast<uint32_t>();
                Duplicate(maxBits, 0xFF800000U, runInfo.vec2MRealSize);
            }
            this->CopyOutState(runInfo, constInfo, vec2ResUb, maxUb, sumUb);
        }
    }
    SetFlag<HardEvent::MTE3_V>(mte3ToVId[0]);
}
    ...
    int64_t calcSize)
{
    ...
    const int64_t scalarOffset = runInfo.attentionOutOffset / constInfo.dSizeV;
    ...
    DataCopyPad(previousMUb, softmaxMaxGm[scalarOffset], scalarParams, scalarPadParams);
    DataCopyPad(previousLUb, softmaxSumGm[scalarOffset], scalarParams, scalarPadParams);
    SetFlag<HardEvent::MTE2_V>(mte2ToVId);
    WaitFlag<HardEvent::MTE2_V>(mte2ToVId);

    // Start the first P copy before the scalar-vector work. Subsequent rows
    // ping-pong the two queue buffers while V computes the current row.
    LocalTensor<float> firstPreviousP = previousPQue.AllocTensor<float>();
    DataCopy(firstPreviousP, partialOutGm[runInfo.attentionOutOffset], stateD);
    previousPQue.EnQue(firstPreviousP);

    // M=max(M1,M2), alpha=exp(M1-M), beta=exp(M2-M), L=alpha*L1+beta*L2.
    // 空集行可能产生NaN权重，但下方仅在L1/L2均有效时读取该行的向量化结果。
    Max(mergeTmpUb, previousMUb, currentM, runInfo.vec2MRealSize);
    Sub(previousDiffUb, previousMUb, mergeTmpUb, runInfo.vec2MRealSize);
    Sub(currentDiffUb, currentM, mergeTmpUb, runInfo.vec2MRealSize);
    Exp(previousWeightUb, previousDiffUb, runInfo.vec2MRealSize);
    Exp(currentWeightUb, currentDiffUb, runInfo.vec2MRealSize);
    Mul(mergeTmpUb, previousWeightUb, previousLUb, runInfo.vec2MRealSize);
    Mul(mergedLUb, currentWeightUb, currentL, runInfo.vec2MRealSize);
    Add(mergedLUb, mergeTmpUb, mergedLUb, runInfo.vec2MRealSize);
    PipeBarrier<PIPE_V>();

    for (int64_t row = 0; row < runInfo.vec2MRealSize; ++row) {
        const int64_t rowPBase = runInfo.attentionOutOffset + row * stateD;
        LocalTensor<float> previousP = previousPQue.DeQue<float>();
        if (row + 1 < runInfo.vec2MRealSize) {
            LocalTensor<float> nextPreviousP = previousPQue.AllocTensor<float>();
            DataCopy(nextPreviousP, partialOutGm[rowPBase + stateD], stateD);
    ...
    this->Bmm2DataCopyOut(runInfo, constInfo, currentP, 0, calcSize);
}
...
__aicore__ inline void QSFAVectorService<TEMPLATE_ARGS>::CopyOutState(
    ...
    LocalTensor<float> &maxUb, LocalTensor<float> &sumUb)
{
    const int64_t stateOffset = runInfo.attentionOutOffset / constInfo.dSizeV;
    ...
    partialParams.srcStride = (qsfaDTemplateAlign64 - constInfo.dSizeV) * sizeof(float);
    partialParams.dstStride = (constInfo.n2G - constInfo.gSize) * constInfo.dSizeV * sizeof(float);
    DataCopyPad(this->partialOutGm[runInfo.attentionOutOffset], partialUb, partialParams);
    ...
    scalarParams.srcStride = 0;
    scalarParams.dstStride = 0;
    DataCopyPad(this->softmaxMaxGm[stateOffset], maxUb, scalarParams);
    DataCopyPad(this->softmaxSumGm[stateOffset], sumUb, scalarParams);
}
    ...
    ConstInfo &constInfo)
{
    if ASCEND_IS_AIV {
        ...
        if constexpr (PROBE_MODE != C8_PROBE_TND_RESOLVE) {
...
__aicore__ inline void QSFAVectorService<TEMPLATE_ARGS>::InitLocalBuffer(TPipe *pipe, ConstInfo &constInfo)
{
    ...
    tPipe->InitBuffer(commonTBuf, 512 * sizeof(float));
    ...
    tPipe->InitBuffer(previousPQue, 2, 512 * sizeof(float));
    ...
    CVSharedParams &sharedParams, int32_t aicIdx, uint8_t subBlockIdx)
{
    ...
    sharedParams.stageMode = sparseAttnSharedkvBaseParams.stageMode;
