...
namespace BaseApi {
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE = false,
    int PROBE_MODE = C8_PROBE_DISABLED>
...
private:
    ...
    __aicore__ inline void InitStagedRequestMetadata(RunParamStr &runParam);
    __aicore__ inline void ApplyQueryValidCount(RunParamStr &runParam);
    ...
    __gm__ int32_t *cacheTokensAddr = nullptr;
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::Init(
    ...
    TPipe *tPipe)
{
    ...
    cacheTokensAddr = (__gm__ int32_t *)cacheTokens;
    ...
    vecBlock.template InitStageOutputs<ENABLE_FINAL_MERGE, PROBE_MODE>(
        missCounts, cacheTokens, partialOut, softmaxMax, softmaxSum,
        attentionOut, constInfo);
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::InitCalcParamsEach()
{
    // 计算总的基本块
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline uint64_t KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::\
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::GetAxisStartIdx(uint32_t bN2EndPrev,
                                                                                uint32_t s1GEndPrev,
                                                                                uint32_t s2EndPrev)
{
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::InitGlobalBuffer(
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::InitMMResBuf(
    __gm__ uint8_t *workspace)
{
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::InitLocalBuffer()
{
    vecBlock.InitLocalBuffer(pipe, constInfo);
}

template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::ComputeConstexpr()
{
    // 计算轴的乘积
    ...
    constInfo.stageMode = sharedParams.stageMode;
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::InitUniqueConstInfo()
{
    // bsize + 1-> bsize
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::Process()
{
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::ProcessMainLoop()
{
    ...
    for (int64_t qsfaBnIdx = qsfaBN2StartIdx; qsfaBnIdx < bN2EndIdx; qsfaBnIdx++) {
        ...
        InitStagedRequestMetadata(runParam);
        ...
        for (int64_t gS1Index = runParam.gs1LoopStartIdx; gS1Index < gS1LoopEnd; gS1Index++) {
            ...
            if (notLastTwoLoop) {
                ...
                ApplyQueryValidCount(runParam);
                bool s2NoNeedCalc =
                    ComputeS2LoopInfo<TEMPLATE_INTF_ARGS>(runParam, this->constInfo);
                // s2不需要算则continue，如果是当前核最后一次循环，则补充计算taskIdx+2的部分
                if (s2NoNeedCalc) {
                    continue;
                }
                if constexpr (IS_SPLIT_G) {
            ...
            for (int64_t s2LoopCount = 0; s2LoopCount <= s2LoopLimit; ++s2LoopCount) {
                if (notLastTwoLoop) {
                    ...
                    if ASCEND_IS_AIC {
                        ...
                        this->vecBlock.template ProcessVec0<ENABLE_FINAL_MERGE, PROBE_MODE>(
                ...
                if (taskId > 1) {
                    if ASCEND_IS_AIV {
                        ...
                        this->vecBlock.template ProcessVec2<ENABLE_FINAL_MERGE, PROBE_MODE>(
                            this->bmm2Buffers.Get(), qsfaRunInfo3, this->constInfo);
                    }
                }
                ++taskId;
            }
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::ComputeAxisIdxByBnAndGs1(
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::InitStagedRequestMetadata(
    RunParamStr &runParam)
{
    ...
    if constexpr (LAYOUT_T != QSFA_LAYOUT::TND) {
        runParam.stagedQueryBegin = runParam.boIdx * constInfo.s1Size;
    }
    if constexpr (!ENABLE_FINAL_MERGE) {
        // ComputeParamBatch has already loaded these two values. Reuse them
        // instead of issuing another pair of request-metadata GM reads.
        runParam.stagedQueryCount = runParam.actualS1Size;
        runParam.stagedResident = runParam.actualS2Size;
        runParam.stagedCacheTokens = cacheTokensAddr[runParam.boIdx];
    } else {
        // Stage2 only consumes its compact miss prefix. Keep request fields
        // initialized without adding resident/cache GM reads to that path.
        runParam.stagedQueryCount = 0;
        runParam.stagedResident = 0;
        runParam.stagedCacheTokens = 0;
    }
}

template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::ApplyQueryValidCount(
    RunParamStr &runParam)
{
    if constexpr (PROBE_MODE == C8_PROBE_BASELINE ||
                  PROBE_MODE == C8_PROBE_PML_WRITE) {
    ...
    const int64_t queryRow =
        runParam.stagedQueryBegin + runParam.s1oIdx;
    const int32_t missCount = missCountsAddr[queryRow];
    runParam.stagedTopkRowOffset = queryRow * 2048LL;
    runParam.stagedMissCount = missCount;
    runParam.stagedHitCount = 0;
    int32_t validCount = missCount;
    if constexpr (PROBE_MODE == C8_PROBE_TND_RESOLVE) {
        const int32_t futureQueries =
            runParam.stagedQueryCount - runParam.s1oIdx - 1;
        const int32_t visibleResident =
            runParam.stagedResident - futureQueries;
        const int32_t cacheTokens = runParam.stagedCacheTokens;
        runParam.stagedHitCount = 2048;
        // The probe intentionally traverses the complete prebuilt tail,
        // including future MTP rows, so its token count exactly matches the
        // legacy full-slot control. Numerical equivalence is out of scope.
        const int32_t tailCount = cacheTokens == 0
            ? 0
            : (runParam.stagedResident > cacheTokens
                ? runParam.stagedResident - cacheTokens
                : 0);
        validCount = cacheTokens == 0
            ? visibleResident
            : 2048 + tailCount;
    } else if constexpr (!ENABLE_FINAL_MERGE) {
        const int32_t localQuery = runParam.s1oIdx;
        const int32_t futureQueries =
            runParam.stagedQueryCount - localQuery - 1;
        const int32_t visibleResident =
            runParam.stagedResident - futureQueries;
        const int32_t cacheTokens = runParam.stagedCacheTokens;
        if (cacheTokens == 0) {
            validCount = visibleResident;
        } else {
            const int32_t hitCount = 2048 - missCount;
            runParam.stagedHitCount = hitCount;
            const int32_t tailCount = visibleResident > cacheTokens
                ? visibleResident - cacheTokens
                : 0;
            validCount = hitCount + tailCount;
        }
    }
    runParam.stagedValidCount = validCount;
    runParam.stagedSlotSetEmpty = validCount <= 0;
    // Empty rows retain one safe placeholder slot so the pipelined kernel
    // still reaches ProcessVec2, where it writes the exact P=0, M=-inf, L=0
    // identity. Non-empty rows must never process the -1 padding after their
    // valid sparse slots.
    runParam.actualS2Size = validCount > 0 ? validCount : 1;
}

template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::SetRunInfo(
    RunInfo &runInfo, RunParamStr &runParam, int64_t taskId, int64_t s2LoopCount, int64_t s2LoopLimit, int64_t multiCoreInnerIdx)
{
    ...
    if (runInfo.multiCoreInnerIdx != multiCoreInnerIdx) {
        ...
        runInfo.stagedTopkRowOffset = runParam.stagedTopkRowOffset;
        runInfo.stagedMissCount = runParam.stagedMissCount;
        runInfo.stagedHitCount = runParam.stagedHitCount;
        runInfo.stagedCacheTokens = runParam.stagedCacheTokens;
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::InitUniqueRunInfo(
...
template <typename CubeBlockType, typename VecBlockType, bool ENABLE_FINAL_MERGE, int PROBE_MODE>
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType, ENABLE_FINAL_MERGE, PROBE_MODE>::ComputeBmm1Tail(
