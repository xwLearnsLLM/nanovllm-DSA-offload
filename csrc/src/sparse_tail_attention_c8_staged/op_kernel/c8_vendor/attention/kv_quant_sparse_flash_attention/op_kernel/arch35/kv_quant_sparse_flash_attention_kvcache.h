    ...
    __gm__ int32_t *actualSeqQlenAddr, __gm__ int32_t * actualSeqKvlenAddr)
{
    ...
    if constexpr (LAYOUT_T == QSFA_LAYOUT::TND) {
        ...
        if (actualSeqQlenAddr != nullptr) {
            const int32_t queryBegin =
                sIdx == 0 ? 0 : actualSeqQlenAddr[sIdx - 1];
            const int32_t queryEnd = actualSeqQlenAddr[sIdx];
            runParam.stagedQueryBegin = queryBegin;
            qsfaActualS1Size = queryEnd - queryBegin;
        } else {
            qsfaActualS1Size = constInfo.s1Size;
            runParam.stagedQueryBegin = sIdx * constInfo.s1Size;
        }
    } else {
