...
struct RunParamStr {  // 分核与切块需要使用到参数
    ...
    // Staged C8 sparse-slot metadata. It is invariant for every S2 tile of
    // one packed query row, so resolve it once before entering the hot KV
    // loop instead of reloading batch metadata for every token.
    int64_t stagedQueryBegin;
    int64_t stagedTopkRowOffset;
    int32_t stagedQueryCount;
    int32_t stagedResident;
    int32_t stagedMissCount;
    int32_t stagedHitCount;
    int32_t stagedCacheTokens;
