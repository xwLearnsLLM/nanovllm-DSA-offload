...
public:
    ...
    __aicore__ inline void InitFutureWorkspace(
        __gm__ uint8_t *futureWorkspace, __gm__ uint8_t *copyCounts,
...
private:
    ...
    __aicore__ inline bool UseFutureWorkspace(
        const RunInfo &runInfo, int64_t &futureTile);
    ...
    GlobalTensor<Q_T> futureWorkspaceGm;
    __gm__ int32_t *copyCountsAddr = nullptr;
...
SFAMatmulService<TEMPLATE_ARGS>::InitFutureWorkspace(
    __gm__ uint8_t *futureWorkspace, __gm__ uint8_t *copyCounts,
    uint32_t sourceFutureWorkspaceMaxMiss,
    uint32_t sourceFutureWorkspaceTileCount)
{
    futureWorkspaceGm.SetGlobalBuffer(
        (__gm__ Q_T *)futureWorkspace);
    copyCountsAddr = (__gm__ int32_t *)copyCounts;
...
SFAMatmulService<TEMPLATE_ARGS>::UseFutureWorkspace(
    const RunInfo &runInfo, int64_t &futureTile)
{
    const int32_t copyCount = copyCountsAddr[runInfo.boIdx];
    if (copyCount <= 0 ||
        copyCount > static_cast<int32_t>(futureWorkspaceMaxMiss) ||
        copyCount > static_cast<int32_t>(runInfo.sparseTokenCount)) {
        return false;
    }
    const int64_t tailEnd = runInfo.sparseTokenCount - copyCount +
        runInfo.tailTokenCount;
    const int64_t firstMissTile = tailEnd / s2BaseSize;
    const int64_t tileStart = runInfo.s2LoopCount * s2BaseSize;
    if (runInfo.s2LoopCount < firstMissTile ||
        tileStart >= tailEnd + copyCount) {
        return false;
    }
    futureTile = runInfo.s2LoopCount - firstMissTile;
    return futureTile >= 0 &&
    ...
    ConstInfo &constInfo)
{
    ...
    int64_t futureTile = 0;
    if (UseFutureWorkspace(runInfo, futureTile)) {
        CopyToL1Nd2Nz<Q_T>(dst,
            futureWorkspaceGm[futureTile * s2BaseSize * dBaseSize],
            runInfo.s2RealSize, dBaseSize, dBaseSize);
    } else {
        CopyToL1Nd2Nz<Q_T>(
            dst, v0ResGmTensor, runInfo.s2RealSize, dBaseSize, dBaseSize);
    }
