...
#include "sfa_impl/nanovllm_sparse_and_tail_attention_template_tiling_key.h"
...
using NanovllmSparseAndTailAttentionTilingDataMla =
    NanovllmSparseAndTailAttentionAndScatterCopyTilingData;

#include "sfa_impl/nanovllm_sparse_and_tail_attention_kernel_mla.h"
...
__aicore__ inline void RunSourceAwareAttention(
    ...
    const NanovllmSparseAndTailAttentionAndScatterCopyTilingData *fusedTiling,
    ...
    TPipe *pipe)
{
    ...
    NanovllmSparseAndTailAttentionMla<FusedType> op;
    const auto *attentionTiling =
        reinterpret_cast<
            const NanovllmSparseAndTailAttentionTilingDataMla *>(
                fusedTiling);
    op.Init(
...
__global__ __aicore__ void nanovllm_sparse_and_tail_attention_and_scatter_copy(
    __gm__ uint8_t *query,
    __gm__ uint8_t *key,
    __gm__ uint8_t *value,
    __gm__ uint8_t *sparseIndices,
