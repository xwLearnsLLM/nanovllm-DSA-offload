#include "nanovllm_sparse_and_tail_attention_and_scatter_copy_tiling.h"
    ...
    uint32_t &attentionMBase)
{
    ...
    OPS_ERR_IF(batchSize == 0 || batchSize > 24,
               OPS_LOG_E(context->GetNodeName(), "This GLM experiment supports 1 <= B <= 24."),
               return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(aicNum < batchSize || aivNum < batchSize * 2U,
               OPS_LOG_E(context->GetNodeName(),
                          "The fused 1AIC+2AIV Attention schedule requires "
                          "one MIX group per request."),
               return ge::GRAPH_FAILED);
...
ge::graphStatus TilingNanovllmSparseAndTailAttentionAndScatterCopy(
    gert::TilingContext *context)
{
    ...
    OPS_ERR_IF(raw == nullptr,
               ...
               return ge::GRAPH_FAILED);
    NanovllmSparseAndTailAttentionAndScatterCopyTilingData fusedTiling;
...
ge::graphStatus TilingPrepareNanovllmSparseAndTailAttentionAndScatterCopy(
...
IMPL_OP_OPTILING(NanovllmSparseAndTailAttentionAndScatterCopy)
    .Tiling(TilingNanovllmSparseAndTailAttentionAndScatterCopy)
    .TilingParse<NanovllmSparseAndTailAttentionAndScatterCopyCompileInfo>(
        TilingPrepareNanovllmSparseAndTailAttentionAndScatterCopy);
