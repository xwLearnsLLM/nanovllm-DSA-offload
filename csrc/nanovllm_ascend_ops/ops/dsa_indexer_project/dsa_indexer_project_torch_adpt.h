    ...
    const std::string& rotary_mode)
{
    ...
    TORCH_CHECK(
        ...
        "rotary_mode must be 'half' or 'interleave'.");

    const uint32_t mode = rotary_mode == "interleave" ? 1U : 0U;
    ...
    auto acl_call =
        [type, stream, q_ptr, cos_ptr, sin_ptr, num_tokens, n_head,
         head_dim, rope_dim, mode, sign_pair_bits, block_dim,
