...
ge::graphStatus FusedLiManageTiling::GetNpuInfo(FusedLiManageTilingInfo &tilingInfo) const
{
    if (context_->GetNodeName() == nullptr) {
        OPS_LOG_E("NanovllmFusedLiManage", "opName got from TilingContext is nullptr.");
...
static ge::graphStatus TilingPrepareForNanovllmFusedLiManage(gert::TilingParseContext * /* context */)
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus TilingForNanovllmFusedLiManage(gert::TilingContext *context)
{
    OPS_ERR_IF(context == nullptr,
               OPS_REPORT_VECTOR_INNER_ERR("NanovllmFusedLiManage", "Tiling context is null."),
               return ge::GRAPH_FAILED);
...
IMPL_OP_OPTILING(NanovllmFusedLiManage)
    .Tiling(TilingForNanovllmFusedLiManage)
