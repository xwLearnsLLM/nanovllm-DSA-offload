...
/*!
 * \file nanovllm_sparse_tail_attention_tiling.cpp
 * \brief
 */
...
#include "../op_kernel/nanovllm_sparse_tail_attention_template_tiling_key.h"
#include "nanovllm_sparse_tail_attention_tiling.h"
...
struct NanovllmSparseTailAttentionCompileInfo {
    int64_t core_num;
};
...
static std::string SFADataTypeToSerialString(ge::DataType type)
{
    ...
    if (it != DATATYPE_TO_STRING_MAP.end()) {
        ...
        OPS_LOG_E("NanovllmSparseTailAttention", "datatype %d not support", type);
        return "UNDEFINED";
    }
}
...
ge::graphStatus TilingNanovllmSparseTailAttention(gert::TilingContext *context)
{
...
ge::graphStatus TilingNanovllmSparseTailAttentionMtp(gert::TilingContext *context)
{
    const ge::graphStatus status = TilingNanovllmSparseTailAttention(context);
...
ge::graphStatus TilingPrepareForNanovllmSparseTailAttention(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SFAInfoParser::GetOpName()
{
    if (context_->GetNodeName() == nullptr) {
        OPS_LOG_E("NanovllmSparseTailAttention", "opName got from TilingContext is nullptr");
        return ge::GRAPH_FAILED;
    }
...
{
    if (context_ == nullptr) {
        OPS_LOG_E("NanovllmSparseTailAttention", "tiling context is nullptr!");
        return ge::GRAPH_FAILED;
    }
    OPS_LOG_FULL(DLOG_INFO, "NanovllmSparseTailAttention", "TilingContext: %s", SFADebugTilingContext(context_).c_str());
...
IMPL_OP_OPTILING(NanovllmSparseTailAttention)
    .Tiling(TilingNanovllmSparseTailAttention)
    .TilingParse<NanovllmSparseTailAttentionCompileInfo>(TilingPrepareForNanovllmSparseTailAttention);

IMPL_OP_OPTILING(NanovllmSparseTailAttentionMtp)
    .Tiling(TilingNanovllmSparseTailAttentionMtp)
    .TilingParse<NanovllmSparseTailAttentionCompileInfo>(TilingPrepareForNanovllmSparseTailAttention);
} // namespace optiling
