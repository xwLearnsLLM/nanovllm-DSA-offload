...
/*!
 * \file qk_score_kernel.h
 * \brief
 */

#ifndef QK_SCORE_KERNEL_H
#define QK_SCORE_KERNEL_H
...
#include "qk_score_service_vector.h"
#include "qk_score_service_cube.h"
...
class QKPreload {
public:
    __aicore__ inline QKPreload(){};
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *weights,
                                __gm__ uint8_t *actualSeqLengthsQ, __gm__ uint8_t *actualSeqLengths,
                                __gm__ uint8_t *blockTable, __gm__ uint8_t *scores, __gm__ uint8_t *workspace,
                                const QkScoreTilingData *__restrict tiling, TPipe *tPipe);
    __aicore__ inline void Process();

    using Q_T = typename QKT::queryType;
    using K_T = typename QKT::keyType;
    using OUT_T = typename QKT::outputType;
    static constexpr bool PAGE_ATTENTION = QKT::pageAttention;
    static constexpr QK_LAYOUT LAYOUT_T = QKT::layout;
    static constexpr QK_LAYOUT K_LAYOUT_T = QKT::keyLayout;

    using MM1_OUT_T = float;

    QKMatmul<QKT> matmulService;
    QKVector<QKT> vectorService;
...
protected:
    ...
    uint64_t scoreOutCoreOffset = 0ULL;
    ...
    QKCommon::ConstInfo constInfo{};
    TempLoopInfo tempLoopInfo{};
    QKCommon::SplitCoreInfo splitCoreInfo{};

    // ================================Init functions==================================
    __aicore__ inline void InitTilingData(const QkScoreTilingData *__restrict tilingData);
    __aicore__ inline void InitBuffers();
    __aicore__ inline void InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ, __gm__ uint8_t *actualSeqLengths);
    // ================================Split Core================================
    __aicore__ inline void SplitCore(uint32_t curCoreIdx, uint32_t &coreNum, QKCommon::SplitCoreInfo &info);
    ...
    __aicore__ inline void ProcessBaseBlock(uint32_t loop, uint64_t s2LoopIdx, QKCommon::RunInfo &runInfo);
    ...
    __aicore__ inline void CalcRunInfo(uint32_t loop, uint32_t s2LoopIdx, QKCommon::RunInfo &runInfo);
    __aicore__ inline void DealActSeqLenIsZero(uint32_t bIdx, uint32_t n2Idx, uint32_t s1Start);
};

template <typename QKT>
__aicore__ inline void QKPreload<QKT>::InitTilingData(const QkScoreTilingData *__restrict tilingData)
{
    ...
    constInfo.s2BaseSize = QKT::s2BaseSize;
    constInfo.s1BaseSize = (constInfo.mBaseSize + constInfo.gSize - 1) / constInfo.gSize;
}

template <typename QKT>
__aicore__ inline void QKPreload<QKT>::InitBuffers()
{
    if ASCEND_IS_AIV {
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ,
...
template <typename QKT>
__aicore__ inline uint32_t QKPreload<QKT>::GetActualSeqLen(uint32_t bIdx, uint32_t actualLenDims, bool isAccumSeq,
                                                           GlobalTensor<uint32_t> &actualSeqLengthsGm,
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::GetS1S2ActualSeqLen(uint32_t bIdx, uint32_t &actS1Size, uint32_t &actS2Size)
{
...
template <typename QKT>
__aicore__ inline uint32_t QKPreload<QKT>::GetS2BaseBlockNumOnMask(uint32_t s1gIdx, uint32_t actS1Size,
                                                                   uint32_t actS2Size)
{
    if (actS2Size == 0) {
        return 0;
...
template <typename QKT>
__aicore__ inline uint32_t QKPreload<QKT>::GetTotalBaseBlockNum()
{
...
template <typename QKT>
__aicore__ void inline QKPreload<QKT>::SplitCore(uint32_t curCoreIdx, uint32_t &coreNum, QKCommon::SplitCoreInfo &info)
{
    ...
    for (uint32_t bN2Idx = 0; bN2Idx < constInfo.batchSize * constInfo.kHeadNum; bN2Idx++) {
        ...
        if constexpr (LAYOUT_T == QK_LAYOUT::BSND) {
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::DealActSeqLenIsZero(uint32_t bIdx, uint32_t n2Idx, uint32_t s1Start)
{
    if ASCEND_IS_AIV {
        if (constInfo.outputLayout == QK_LAYOUT::TND) {
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *weights,
                                            __gm__ uint8_t *actualSeqLengthsQ, __gm__ uint8_t *actualSeqLengths,
                                            __gm__ uint8_t *blockTable, __gm__ uint8_t *scores,
                                            __gm__ uint8_t *workspace, const QkScoreTilingData *__restrict tiling,
                                            TPipe *tPipe)
{
    if ASCEND_IS_AIV {
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::GetBN2Idx(uint32_t bN2Idx)
{
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::CalcS2LoopParams(uint32_t bN2LoopIdx, uint32_t gS1LoopIdx)
{
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::CalcGS1LoopParams(uint32_t bN2LoopIdx)
{
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::CalcRunInfo(uint32_t loop, uint32_t s2LoopIdx, QKCommon::RunInfo &runInfo)
{
    runInfo.loop = loop;
    ...
    runInfo.actualSingleProcessSInnerSizeAlign =
        QKCommon::Align((uint32_t)runInfo.actualSingleProcessSInnerSize, QKCommon::ConstInfo::BUFFER_SIZE_BYTE_32B);
    ...
    if (runInfo.isFirstS2InnerLoop) {
        ...
        if constexpr (LAYOUT_T == QK_LAYOUT::TND) {
    ...
    * constInfo.headDim;
    ...
    runInfo.scoreOutOffset = scoreOutCoreOffset;
}

template <typename QKT>
__aicore__ inline void QKPreload<QKT>::Process()
{
    if (usedCoreNum == 0) {
        ProcessInvalid();
        return;
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::ProcessInvalid()
{
    if ASCEND_IS_AIV {
        ...
        if (baseSize < totalOutputSize) {
            ...
            vectorService.CleanInvalidOutput(baseSize, dealSize);
        }
    }
}

template <typename QKT>
__aicore__ inline void QKPreload<QKT>::ProcessMain()
{
    ...
    if ASCEND_IS_AIV {
        CrossCoreSetFlag<QKCommon::ConstInfo::FIA_SYNC_MODE2, PIPE_MTE2>(constInfo.syncV1C1);
        CrossCoreSetFlag<QKCommon::ConstInfo::FIA_SYNC_MODE2, PIPE_MTE2>(constInfo.syncV1C1);
    } else {
        matmulService.AllocEventID();
    }

    QKCommon::RunInfo runInfo;
    uint32_t gloop = 0;
...
template <typename QKT>
__aicore__ inline void QKPreload<QKT>::ProcessBaseBlock(uint32_t loop, uint64_t s2LoopIdx, QKCommon::RunInfo &runInfo)
{
    CalcRunInfo(loop, s2LoopIdx, runInfo);
    if ASCEND_IS_AIC {
        CrossCoreWaitFlag(constInfo.syncV1C1);
        matmulService.ComputeMm1(runInfo);
        CrossCoreSetFlag<QKCommon::ConstInfo::FIA_SYNC_MODE2, PIPE_FIX>(constInfo.syncC1V1);
    } else {
