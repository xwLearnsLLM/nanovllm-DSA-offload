#ifndef NANOVLLM_LIGHTNING_INDEXER_DECODE_UPDATE_MTP_TORCH_ADPT_H_
#define NANOVLLM_LIGHTNING_INDEXER_DECODE_UPDATE_MTP_TORCH_ADPT_H_

namespace vllm_ascend {

inline void npu_lightning_indexer_decode_update_mtp_out(
  ...
  EXEC_NPU_CMD_ORDERED(
      aclnnNanovllmLiduDecodeUpdateMtp,
      keepalive,
      query,
      key,
      weights,
...
npu_lightning_indexer_decode_update_mtp(
    const at::Tensor& query,
    const at::Tensor& key,
    const at::Tensor& weights,
    const at::Tensor& req_pool_entries,
  ...
  npu_lightning_indexer_decode_update_mtp_out(
      query, key, weights, req_pool_entries, cache_slots, cache_tokens,
      candidate_lens, block_table, topk_slots, miss_source_ids,
      miss_destination_slots, miss_counts);
