...
/*!
 * \file nanovllm_sparse_and_tail_attention_tiling.cpp
 * \brief
 */
...
#include "../op_kernel/nanovllm_sparse_and_tail_attention_template_tiling_key.h"
#include "nanovllm_sparse_and_tail_attention_tiling.h"
...
static std::string SFADataTypeToSerialString(ge::DataType type)
{
    ...
    if (it != DATATYPE_TO_STRING_MAP.end()) {
        ...
        OPS_LOG_E("NanovllmSparseAndTailAttention", "datatype %d not support", type);
        return "UNDEFINED";
    }
}
...
ge::graphStatus SFAInfoParser::GetOpName()
{
    if (context_->GetNodeName() == nullptr) {
        OPS_LOG_E("NanovllmSparseAndTailAttention", "opName got from TilingContext is nullptr");
        return ge::GRAPH_FAILED;
    }
...
{
    if (context_ == nullptr) {
        OPS_LOG_E("NanovllmSparseAndTailAttention", "tiling context is nullptr!");
        return ge::GRAPH_FAILED;
    }
    OPS_LOG_FULL(DLOG_INFO, "NanovllmSparseAndTailAttention", "TilingContext: %s", SFADebugTilingContext(context_).c_str());
