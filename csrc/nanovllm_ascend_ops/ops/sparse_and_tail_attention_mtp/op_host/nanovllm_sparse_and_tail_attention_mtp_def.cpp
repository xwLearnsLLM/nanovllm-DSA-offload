...
namespace ops {
class NanovllmSparseAndTailAttentionMtp : public OpDef {
public:
    explicit NanovllmSparseAndTailAttentionMtp(const char *name) : OpDef(name)
    {
        this->Input("query")
        ...
        this->Input("sparse_indices")
            .ParamType(REQUIRED).DataTypeList({ge::DT_INT32})
            .FormatList({ge::FORMAT_ND}).AutoContiguous();
...
OP_ADD(NanovllmSparseAndTailAttentionMtp);
} // namespace ops
