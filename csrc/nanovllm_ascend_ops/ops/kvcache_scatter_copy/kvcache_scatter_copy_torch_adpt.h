...
inline void npu_kvcache_scatter_copy(
  ...
  TORCH_CHECK(source_token_ids.sizes() == destination_slots.sizes(),
  ...
  TORCH_CHECK(hbm_block_table.scalar_type() == at::kInt &&
                  ...
                  source_token_ids.scalar_type() == at::kInt &&
  ...
  TORCH_CHECK(hbm_k_rope.is_contiguous() && hbm_kv_cache.is_contiguous() &&
                  ...
                  source_token_ids.is_contiguous() && destination_slots.is_contiguous() &&
  ...
  TORCH_CHECK(hbm_kv_cache.device() == device && dram_k_rope.device() == device &&
                  ...
                  dram_block_table.device() == device && source_token_ids.device() == device &&
                  destination_slots.device() == device && copy_counts.device() == device,
