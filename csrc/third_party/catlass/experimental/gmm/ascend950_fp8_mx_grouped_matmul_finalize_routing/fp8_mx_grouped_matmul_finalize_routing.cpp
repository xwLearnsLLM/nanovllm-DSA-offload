...
struct GroupedGemmFinalizeRoutingOptions : public GroupedGemmOptions {
    const std::string HELPER = "problem_count m n k trans_b group_list_type enable_bias batch data_parallel_size enable_shared_input shared_input_weight shared_input_offset quant_type [device_id]]]";
...
template <class ElementA, class ElementB,
    ...
    bool transB = false, bool enableBias = false, bool enableSharedInput = false
>
void MxGroupedMatmulFinalizeRouting(Options const &options) {
    ...
    static_assert((std::is_same_v<ElementA, float8_e5m2_t> || std::is_same_v<ElementA, float8_e4m3_t>) &&
        (std::is_same_v<ElementB, float8_e5m2_t> || std::is_same_v<ElementB, float8_e4m3_t>) && std::is_same_v<ElementMxScale, float8_e8m0_t>,
    ...
    if constexpr (enableSharedInput) {
        ACL_CHECK(aclrtMalloc(reinterpret_cast<void **>(&deviceSharedInput), sizeSharedInput, ACL_MEM_MALLOC_HUGE_FIRST));
        ACL_CHECK(aclrtMemcpy(deviceSharedInput, sizeSharedInput, hostSharedInput.data(), sizeSharedInput, ACL_MEMCPY_HOST_TO_DEVICE));
    }
    ...
    const auto cleanupAndFinalize = [&]() {
        ...
        if (deviceBias) ACL_CHECK(aclrtFree(deviceBias));
        if (deviceLogit) ACL_CHECK(aclrtFree(deviceLogit));
        if (deviceRowIndex) ACL_CHECK(aclrtFree(deviceRowIndex));
        if (deviceOut) ACL_CHECK(aclrtFree(deviceOut));
        if (deviceSharedInput) ACL_CHECK(aclrtFree(deviceSharedInput));
        if (deviceWorkspace) ACL_CHECK(aclrtFree(deviceWorkspace));
    ...
    using BlockEpilogue = Epilogue::Block::BlockEpilogueFinalizeRouting<EpilogueDispatchPolicy, ArchTag, vecTileShape, ElementC, ElementRowIndex, ElementSharedInput>;
    using BlockScheduler = typename Gemm::Block::ColumnBlockSwizzle;
    using MatmulKernel = Gemm::Kernel::GroupedMxMatmulFinalizeRoutingTla<BlockMmad,
            BlockEpilogue, BlockScheduler, ElementGroupList, ElementSharedInput>;
    using MatmulAdapter = Gemm::Device::DeviceGemm<MatmulKernel>;
    typename MatmulKernel::Arguments arguments{aicCoreNum, options.problemShape, options.problemCount,
        deviceGroupList, deviceA, layoutA, deviceB, layoutB,
        deviceMxScaleA, layoutMxScaleA, deviceMxScaleB, layoutMxScaleB,
        deviceWorkspace, layoutC,
        deviceBias,
        deviceLogit, deviceRowIndex,
        deviceSharedInput, groupListType, sharedInputWeight, sharedInputOffset,
        batch, bsdp, deviceOut};
...
void Run(Options const &options) {
    if (options.quantDataType == "float8_e4m3fn") {
        MxGroupedMatmulFinalizeRouting<float8_e4m3_t, float8_e4m3_t, Shape<Int<256>,Int<256>,Int<256>>, Shape<Int<256>,Int<256>,Int<128>>, TB, EnableBias, EnableSharedInput>(options);
    } else if (options.quantDataType == "float8_e5m2") {
        MxGroupedMatmulFinalizeRouting<float8_e5m2_t, float8_e5m2_t, Shape<Int<256>,Int<256>,Int<256>>, Shape<Int<256>,Int<256>,Int<128>>, TB, EnableBias, EnableSharedInput>(options);
    } else {
