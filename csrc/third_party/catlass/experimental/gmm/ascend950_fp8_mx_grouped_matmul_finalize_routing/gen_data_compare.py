...
def gen_data_fp8(m, n, k, trans_a, trans_b, problem_count, batch, data_parallel_size,
                 enable_bias, enable_shared_input, shared_input_weight, shared_input_offset,
                 group_list_type, quant_type=torch.float8_e5m2):
    ...
    if base_size >= 1 and problem_count > 1:
        ...
        group_sizes = [base_size + (1 if i >= problem_count - remainder else 0) for i in range(problem_count)]
    ...
    if enable_bias:
        ...
        bias_np = torch.tensor(bias.flatten().untyped_storage(), dtype=torch.int8).numpy()
        bias_np.tofile('./data/bias.bin')

    shared_input = None
    if enable_shared_input:
        shared_input = torch.randn(bsdp, n, dtype=torch.bfloat16)
        shared_input_np = torch.tensor(shared_input.flatten().untyped_storage(), dtype=torch.int8).numpy()
    ...
    if enable_shared_input and shared_input is not None:
        ...
        out_upgrade[shared_input_offset:(shared_input_offset + bsdp), :] += contribution
...
if __name__ == "__main__":
    ...
    quant_torch_type = torch.float8_e5m2 if quant_type == "float8_e5m2" else torch.float8_e4m3fn
    golden_data, golden_upgrade = gen_data_fp8(
        m, n, k, 0, trans_b, problem_count,
        batch, dp, enable_bias, enable_shared_input,
        shared_input_weight, shared_input_offset, group_list_type,
    ...
    result = subprocess.run(
        [op_path, str(problem_count), str(m), str(n), str(k), str(trans_b),
         str(group_list_type), str(enable_bias), str(batch), str(dp),
         str(enable_shared_input), str(shared_input_weight), str(shared_input_offset),
         quant_type, str(device_id)],
        capture_output=True,
        text=True
    )
    ...
    print("------ 计算相对误差 -----")
    result_data = np.fromfile(f"./data/result.bin", dtype=np.float32).reshape(batch, n)
