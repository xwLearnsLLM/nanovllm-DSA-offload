...
class GroupGemm(OperationBase):
    ...
    def __init__(
        ...
        if A is not None and B is not None:
            ...
            if len(A_shape) != 2 or len(B_shape) != 3:
                raise ValueError("A must be 2D tensor (`m, k`) and B must be 3D tensor (`problemCount, k, n`) for group gemm")
    ...
    def get_kernels(
        ...
        if final_groupList is None:
            raise ValueError("groupList must be provided, either in __init__ or get_kernels")
        
        groupList_shape, groupList_element, _, _ = extract_info(final_groupList, None, None)
        if groupList_shape is None:
            raise ValueError("groupList must have valid shape")
        ...
        use_init_values = (A is None and B is None) and (self.A is not None and self.B is not None)
        
        if use_init_values:
            # 使用 __init__ 中保存的值
            if self.M is None or self.K is None or self.N is None:
                raise ValueError("无法确定 M, K, N，请确保在 GroupGemm.__init__ 中传入了 A 和 B OpTensor")
            ...
            if A is None or B is None:
                raise ValueError("A 和 B 必须提供，可以通过 GroupGemm.__init__ 或 get_kernels 参数传入")
            
            A_shape, element_A, layout_A, A_tensor = extract_info(A, self.element_A, self.layout_A)
            B_shape, element_B, layout_B, B_tensor = extract_info(B, self.element_B, self.layout_B)
            Bias_shape, element_Bias, layout_Bias, Bias_tensor = extract_info(Bias, self.element_Bias, self.layout_Bias)
            C_shape, element_C, layout_C, C_tensor = extract_info(C, self.element_C, self.layout_C)
