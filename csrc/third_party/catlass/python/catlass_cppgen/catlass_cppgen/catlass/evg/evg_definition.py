...
def EliminateDupAndDeadNodes(dag: EpilogueVisitorGraph):
    ...
    def simplify_graph(signature_map):
        ...
        for node in signature_map:
            if isinstance(node, LoadNode) or not signature_map[node]:
                continue
            for node_compare in signature_map:
                if node == node_compare:
                    continue
                if Counter(signature_map[node]) == Counter(signature_map[node_compare]):
                    for input_node in signature_map[node_compare]:
                        dag.remove_edge(input_node, node_compare)
                    for output_node in dag.get_outputs(node_compare):
                        dag.remove_edge(node_compare, output_node)
                        dag.add_edge(node, output_node)
                    dag.remove_node(node_compare)
                    modified = True
                    signature_map = {}
                    for node_tmp in list(dag.topological_nodes()):
                        node_info = tuple(dag.get_inputs(node_tmp))
                        signature_map[node_tmp] = node_info
                    break
                if modified:
                    break
            if modified:
...
def DynamicShapeTransfer(dag: EpilogueVisitorGraph):
    ...
    """
    for node in dag.topological_nodes():
        # Within dynamic shape mode, the symbolic shape should be a sympy.Symbol or
        # a sympy expression containing Symbol(s) (e.g. 2*s20). Skip nodes whose shape
        # is fully static (no free symbols at all).
        if all(
            not (isinstance(shape_i, Expr) and shape_i.free_symbols)
            for shape_i in node.metadata.shape
        ):
            continue
        symbolic_shape = []
        # Replace longer keys first so that a compound expression like "2*s20" is
        ...
        for shape_i in node.metadata.shape:
            shape_i = str(shape_i)
            for key, item in sorted_substitution:
                shape_i = shape_i.replace(key, item)
...
def DAG2Tree(dag: EpilogueVisitorGraph):
    ...
    def _fuse_subgraph(nodes_to_fuse: Set[NodeBase], lca: NodeBase):
        ...
        lca_output_count = len(dag.get_outputs(lca))
        ...
        tv_node = TopoVisitorNode(
            ...
            subgraph=subgraph,
            output_node=lca,
            lca_output_count=lca_output_count,
        )
...
class EVGDef:
    ...
    def def_tree_node(self, node: NodeBase):
        ...
        inputs_str = ",\n".join(
            f"    {self.get_visitor_name(i_node)}" for i_node in sorted_input_nodes
        )
        tree_node_str = f"""
...
"""
    ...
    def generate_node_args(
        ...
        if isinstance(node, TopoVisitorNode):
            ...
            for i, value in enumerate(tree_result_list):
