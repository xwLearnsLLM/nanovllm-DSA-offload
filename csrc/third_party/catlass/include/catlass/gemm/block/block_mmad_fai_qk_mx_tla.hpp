...
>
struct BlockMmadTla <
    MmadFAIQKMx<Arch::Ascend950, PAGED_CACHE_FLAG_, ENABLE_UNIT_FLAG_>,
    L1TileShape_,
    L0TileShape_,
    ElementA_,
    ElementB_,
...
public:
    ...
    static constexpr uint32_t BLOCK_L1_SIZE = (L1A_TILE_SIZE + L1B_TILE_SIZE + L1SCALEA_TILE_SIZE + L1SCALEB_TILE_SIZE) * STAGES;
...
public:
    ...
    BlockMmadTla(Arch::Resource<ArchTag> &resource, uint32_t& l1BufAddrStart, uint32_t& l0CBufAddrStart)
    {
        ...
        for (uint32_t i = 0; i < STAGES; i++) {
            ...
            l1MxScaleATensorList_[i] = resource.l1Buf.template GetBufferByByte<ElementMxScaleA>(l1MxScaleAOffset + L1SCALEA_TILE_SIZE * i);
            l1MxScaleBTensorList_[i] = resource.l1Buf.template GetBufferByByte<ElementMxScaleB>(l1MxScaleBOffset + L1SCALEB_TILE_SIZE * i);
...
public:
    template<class TensorA, class TensorB, class TensorC, class Shape, 
        class TensorMxScaleA, class TensorMxScaleB>
    CATLASS_DEVICE
    void operator()(
        TensorA& tensorA,
        TensorB& tensorB,
        TensorC& tensorC,
        TensorMxScaleA& tensorMxScaleA,
        TensorMxScaleB& tensorMxScaleB,
        ...
        bool isFirstLoop = false, bool isLastLoop = false)
    {
        ...
        for (int32_t kIdx = 0; kIdx < kLoops; ++kIdx) {
            ...
            auto layoutMxScaleAInL1 = tla::MakeMxScaleLayout<ElementMxScaleA, LayoutTagL1MxScaleA, false>(blockM, blockK / MX_SCALE_GROUP_NUM);
            auto tensorL1MxScaleA = tla::MakeTensor(l1MxScaleATensorList_[l1AListId_], layoutMxScaleAInL1, Arch::PositionL1{});
            auto tensorL1TileMxScaleA = GetTile(
                tensorL1MxScaleA,
                tla::MakeCoord(0, kIdx * L0_TILE_K / MX_SCALE_GROUP_NUM),
                tla::MakeShape(blockM, tileK / MX_SCALE_GROUP_NUM)
            );
            auto tensorTileMxScaleA = GetTile(
                tensorMxScaleA, tla::MakeCoord(0, kIdx * L0_TILE_K / MX_SCALE_GROUP_NUM), tla::MakeShape(blockM, CeilDiv<MX_SCALE_GROUP_NUM>(tileK))
            );
            ...
            auto layoutMxScaleBInL1 = tla::MakeMxScaleLayout<ElementMxScaleB, LayoutTagL1MxScaleB, true>(tileK / MX_SCALE_GROUP_NUM, blockN);
            ...
            auto tensorTileMxScaleB = GetTile(
                tensorMxScaleB, tla::MakeCoord(kIdx * L0_TILE_K / MX_SCALE_GROUP_NUM, 0), tla::MakeShape(CeilDiv<MX_SCALE_GROUP_NUM>(tileK), blockN)
            );
