# 950BasicMatmul(NoTLA) Example Readme

## 功能说明

Ascend950代际下的基础矩阵乘算子(AtlasA2代际下请参考[00_basic_matmul_notla](../00_basic_matmul/README.md))，使用非TLA组件（Ascend950下基于TLA组件实现的基础矩阵乘算子详见[43_ascend950_basic_matmul](../43_ascend950_basic_matmul/README.md)），代际兼容模式。
...
```
├── 55_ascend950_basic_matmul_notla
...
1. 编译样例代码，并编译生成相应的算子可执行文件(编译时需加`-DCATLASS_ARCH=3510`)。

```
bash scripts/build.sh -DCATLASS_ARCH=3510 55_ascend950_basic_matmul_notla
```
