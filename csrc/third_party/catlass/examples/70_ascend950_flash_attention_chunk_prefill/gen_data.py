...
class TestFlashAttentionInfer():
    ...
    def ref_flash_attention(
        ...
        if kv_start < context_len:
            ...
            if kv_start == 0:
                gm = None
            p_result, row_sum, dm, gm = self.softmax1(qk_result, kv_start == 0, gm, is_kvs_last_loop)
            p_result = p_result.astype(data_type)
            if kv_start == 0:
                gm_high = None
            p_result_high, row_sum_high, dm_high, gm_high = self.softmax1(qk_result_high, kv_start == 0, gm_high, is_kvs_last_loop)
        ...
        for kv_start in range(kv_start, context_len, context_size):
            ...
            p_result, row_sum, dm, gm = self.softmax1(qk_result, False, gm, is_kvs_last_loop)
            p_result = p_result.astype(data_type)
            p_result_high, row_sum_high, dm_high, gm_high = self.softmax1(qk_result_high, False, gm_high, is_kvs_last_loop)
            lo = self.group_matmul(p_result.shape[0], sub_value.shape[0], p_result, sub_value)
            lo_high = self.group_matmul(p_result.shape[0], sub_value.shape[0], p_result.astype(np.float32), sub_value.astype(np.float32))
    ...
    def ref_single_query_cached_kv_attention(self, attention_inputs: AttentionInputs, output, golden_gpu_output, golden_lse_output, golden_gpu_lse_output) -> None:
        ...
        for i in range(batch):
            ...
            out = out_normal if inner_prec == 0 else out_low_prec
    ...
    def calc_data(self, gen_data_params: GenDataParams):
        ...
        attention_inputs = self.AttentionInputs(query, key_cache, value_cache, block_tables,
            ...
            gen_data_params.inner_prec, gen_data_params)
...
if __name__ == "__main__":
    ...
    # 参数顺序：batch, qSeqlen, kvSeqlen, numHeads, kvHeads, qkHeadSize, vHeadSize,
    #              isVariedLen, dtype, numblocks, innerPrec, blocksize, cacheLayout
    ...
    inner_prec = int(sys.argv[11])
    block_size = int(sys.argv[12])
    cache_layout = str(sys.argv[13])

    q_seqlen_list, kv_seqlen_list = gen_seqlen(q_seqlen, kv_seqlen, is_varied_len, batch)
    max_kv_seqlen = max(kv_seqlen_list)

    required_min_blocks = batch * ((max_kv_seqlen + block_size - 1) // block_size)
    if num_blocks < required_min_blocks:
        print(f"[ERROR] numBlocks ({num_blocks}) is less than minimum required "
              f"= batch * ceil(maxKvSeqlen / blockSize) = {batch} * "
              f"({max_kv_seqlen} / {block_size}) = {required_min_blocks}")
        sys.exit(1)
    ...
    gen_data_params = testObj.GenDataParams(
        ...
        num_blocks, block_size, dtype, inner_prec, 
        q_seqlen, kv_seqlen, cache_layout,
        core_num=24
    )
