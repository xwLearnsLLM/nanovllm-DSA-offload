...
#include "catlass/gemm/kernel/matmul_full_loadA_tla.hpp"
...
static void Run(const Options &options)
{
    ...
    using DispatchPolicy = Gemm::MmadAscend950FullLoadA<
        ArchTag,
    ...
    if (m > tla::get<0>(L1TileShape{})) {
        ...
        // kernel level
        using MatmulKernel = Gemm::Kernel::MatmulFullLoadATla<BlockMmad, BlockEpilogue, BlockScheduler>;
        ...
        if (status == Status::kInvalid) {
            std::cout << "Not satisfy the constraints of 73_ascend950_matmul_full_loadA." << std::endl;
