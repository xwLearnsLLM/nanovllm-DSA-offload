...
static void Run(const Options &options)
{
    ...
    size_t lenMxScaleA =
        static_cast<size_t>(static_cast<uint64_t>(m) * static_cast<uint64_t>(mxScaleAlignedK));
    size_t lenMxScaleB =
        static_cast<size_t>(static_cast<uint64_t>(mxScaleAlignedK) * static_cast<uint64_t>(n));
    ...
    uint64_t taskNum64 = static_cast<uint64_t>(CeilDiv(options.problemShape.m(), tla::get<0>(L1TileShape{}))) *
                         static_cast<uint64_t>(CeilDiv(options.problemShape.n(), tla::get<1>(L1TileShape{})));
